...
def обучение_модели(модель, x_train, y_train, x_test=[], y_test=[], 
  ...
  if 'количество' in kwargs:
    ...
    stage_history = []
    for start in range(kwargs['количество']):
      result = ''
      idx_best = 0
      best_result = 0
      ...
      def on_epoch_end(epoch, log):
        ...
        p3 = p1 + ' Точность на обучающей выборке: ' + str(round(log[k[1]]*100,2))+'%'
        if len(k)>2:
...
def создать_выборки_вакансии():
  ...
  x_train, x_val, y_train, y_val, idx_train, idx_val = train_test_split(data, labels, indexes, test_size = 0.2, shuffle = True)
