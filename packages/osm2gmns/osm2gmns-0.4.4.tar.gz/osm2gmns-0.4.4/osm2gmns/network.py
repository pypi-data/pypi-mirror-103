...
def _parseNodes(network, nodes, strict_mode):
    ...
    network.central_lon, network.central_lat = float(central_lon), float(central_lat)
    network.northern = True if central_lat >= 0 else False
...
def _parseWays(network, ways, relations, offset, network_type, POIs):
    osm_way_dict = {}
...
def _parseOSM(network, nodes, ways, relations, strict_mode, offset, network_type, POIs):
    _parseNodes(network, nodes, strict_mode)
    _parseWays(network, ways, relations, offset, network_type, POIs)


def _buildNet(netdata,network_type_valid, POIs, strict_mode, offset, min_nodes, combine, int_buffer, bbox, default_lanes, default_speed):
    network = Network()
    ...
    _parseOSM(network, nodes, ways, relations, strict_mode, offset, network_type_valid, POIs)
...
def getNetFromOSMFile(osm_filename='map.osm', network_type=('auto',), POIs=False, strict_mode=True, offset=0, min_nodes=1,
    ...
    network = _buildNet(netdata,network_type_valid, POIs,strict_mode, offset, min_nodes, combine, int_buffer, bbox, default_lanes, default_speed)
    return network


def getNetFromPBFFile(pbf_filename='map.osm.pbf', network_type=('auto',), POIs=False, strict_mode=True, offset=0, min_nodes=1,
                      combine=False, int_buffer=default_int_buffer, bbox=None, default_lanes=False, default_speed=False):
    """
    :param pbf_filename:
    :param network_type:
