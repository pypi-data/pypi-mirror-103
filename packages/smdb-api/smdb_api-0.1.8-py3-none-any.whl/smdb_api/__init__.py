...
class Message:
    ...
    def from_json(json):
        return Message(json["sender"], json["content"], json["channel"], [Attachment.from_json(attachment) for attachment in json["attachments"]], json["called"])

    def __init__(self, sender, content, channel, attachment, called):
        self.sender = sender
    ...
    def to_json(self):
        return {"sender":self.sender, "content":self.content, "channel":self.channel, "called":self.called, "attachment":[attachment.to_json() for attachment in self.attachments] if len(self.attachment) > 0 else None}
