...
class ItemReserve(SalesInvBase):
    ...
    def create_reserve(cls,docRef, customer_id, location_id, item_id, reserved, finish_week, reserveDate=None, inventory_location=None):
        ...
        ir.location = ir._get_loc_info(ir._get_sb_by_dnl(location_id))
        ...
        if inventory_location is None:
            ...
            ir._inventory_location = reserveInfo['location'].get('inventory_location',None)
            if ir._inventory_location is None:
                # otherwise default the inventory location
                ir._inventory_location = InventoryLocation.getInstance().default_location
        else:
            ir._inventory_location = inventory_location
    ...
    def update_reserve(self, customer_id, location_id, item_id, reserved, gwParent, reserveDate=None, inventory_location=None):
        ...
        if inventory_location is None:
            # if it wasn't passed in...grab it from the location
            if isinstance(reserveInfo['location'],StorageBlob):
                self._inventory_location = getattr(reserveInfo['location'],'inventory_location',None)
            if isinstance(reserveInfo['location'],dict):
                self._inventory_location = reserveInfo['location'].get('inventory_location',None)
            if self._inventory_location is None:
                # otherwise default the inventory location
                self._inventory_location = InventoryLocation.getInstance().default_location
        else:
    ...
    def refresh(self,refreshInvLoc=False):
        ...
        if refreshInvLoc:
            tempLoc = getattr(reserveInfo['location'],'inventory_location',None)
            if tempLoc is None:
                logging.error(f"ERROR--ERROR--ERROR: Location {self.location_id}, has no inventory location set.. defaulting")
                tempLoc = InventoryLocation.getInstance().default_location
            self._inventory_location = tempLoc
            if self._inventory_location is None:
                # otherwise default the inventory location
                logging.error(f"WARNING--WARNING--WARNING:  No inventory location set for location: {self.location_id}... defaulting")
                self._inventory_location = InventoryLocation.getInstance().default_location
