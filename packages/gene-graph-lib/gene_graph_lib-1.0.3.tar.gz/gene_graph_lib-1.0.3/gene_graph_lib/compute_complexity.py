...
class GenomeGraph:
	...
	def read_graph(self, file=None, names_list='all', generate_freq=False):
		...
		file : str
			...
			Exaple file:
			GENE0001 GENE0002 Genome1 Contig1 start0001 end0001 start0002 end0002	(divided by space)
			...
			GENE3998 GENE3999 Genome1 Contig1 start3998 end3998 start3999 end3999
			GENE3999 GENE4000 Genome1 Contig1 start3999 end3999 start4000 end4000
			...
		...
		for line in f_in:
			...
			if stamm not in self.genes_info:
				self.genes_info[stamm] = {}

			if contig not in self.genes_info[stamm]:
				self.genes_info[stamm][contig] = {}
			...
			self.genes_info[stamm][contig][start_gene] = start_coord
			self.genes_info[stamm][contig][end_gene] = end_coord
		...
		for edge in edge_table:
			...
			if current_contig == None or current_contig != contig or current_name != name:
	...
	def _save_bed(self, data, outdir, contig, name):

		f_wc = open(outdir + '/window_complexity.bed', 'a+')
		for gene in data[0]:
			gene_coords = self.genes_info[name][contig][self.genes_decode[gene]]
			f_wc.write(
	...
	def _save_data(self, data, outdir, contig, name):
	...
	def compute_complexity(self, outdir, reference, window=20, iterations=500, min_depth=0, max_depth=-1, save_db=None):
		...
		for contig in self.list_graph[reference]:
			...
			self._save_data([complexity_table, io_table, all_bridges, 
						prob_complexity_table, prob_io_table, prob_all_bridges], outdir, contig, reference)
