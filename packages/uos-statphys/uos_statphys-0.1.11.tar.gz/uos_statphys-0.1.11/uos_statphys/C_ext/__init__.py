import platform, os, ctypes, pkg_resources, sys, gc
import subprocess
import numpy as np
...
class cuda_functions:
    def __init__(self, func):
        self._func = func
        self._block, self._thread = None, None
    
    def __getitem__(self, item):
        temp = self.__class__(self._func)
        temp._block, temp._thread = item
        return temp
...
def precompile(filename, tempname, encoding = 'utf8', debug = False):
    cu = (os.path.splitext(filename)[1].lower() == "cu")
    ...
    with  open(filename, encoding=encoding) as cppsource:
        ...
        for i, line in enumerate(cppsource):
            ...
            if line[:18] == "//@python_cuda_def":
                if not cu:
                    raise NotImplementedError("python_cuda_def method is only for .cu file!")
                doc, funcdef  = function_def_check(cppsource)
                cu_docs.append(doc)
                cu_functions.append(")".join(funcdef.split(")")[:-1])+");\r\n")
        if header_line == -1:
            raise SyntaxError("Cannot find '//@extern_python' from source.")

    #C functions organizing
    func_dict = {}
    cu_func_dict = {}
    doc_dict = {}
    if debug:
    ...
    for docstring, func in zip(docs, functions):
        ...
        if debug:
            print(f"\t{name}")
    #cuda functions organizing
    if debug:
        print("Cuda functions found from source : ")
    for docstring, func in zip(cu_docs, cu_functions):
        name, types = cdefs_analyzer(func)
        cu_func_dict[name] = types
        doc_dict[name] = docstring
        if debug:
            print(f"\t{name}")
            ...
            for i,line in enumerate(cppsource):
                if i<header_line:
                    ...
                    if func_name in func_dict:
                        ...
                        if func_name in cu_func_dict:
                            continue
                        cpp_source.write(line)
                elif i == header_line:
                    for cudefs in cu_functions:
                        cpp_source.write(cudefs)
                    if cpp:
                    ...
                    for cudefs in cu_functions:
                        cufunc = cudefs.split("(")
                        cufunc[0] = cufunc[0].rstrip() + "_py"
                        cufunc[0] = cufunc[0].split()[-2:]
                        cufunc[1] = "int* ___block, int* ___thread, " + cufunc[1]    ##### ___ 3"_" block and thread must be length 3 array always
                        newdef = ' ('.join(cufunc)                   ## cufunc : __device__ (restype) some_func(args) -> newdef : (restype) some_func_py(args)
                        ")".join(newdef.split(")")[:-1])
                        cpp_source.write(newdef)  ###################  declaration
                        cpp_source.write("""{
                            dim3 tpb (___thread[0],___thread[1],___thread[2])
                            dim3 bpg (___block[0],___block[1],___block[2])
                            """)

                        cufunc = cudefs.split("(")
                        cufunc[0] += "<<<bpg, tpb>>>"
                        cpp_source.write("(".join(cufunc))
                        cpp_source.write
                        pass ### cuda function wrapper
                    if cpp:
                        cpp_source.write('};\r\n')
