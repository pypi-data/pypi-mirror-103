...
class Mito extends React.Component<MitoProps, MitoState> {
    ...
    */
    displayTour(): TourName[] {
        ...
        // If user has not taken the explore tour, but intends to explore their data or filter/sort, show them the explore tour
        if (!this.state.tourJSON.receivedTours.includes(TourName.EXPLORE_DATA) && (
            this.state.tourJSON.intendedBehavior.includes(QuestionAnswers.EXPLORE_DATA) || this.state.tourJSON.intendedBehavior.includes(QuestionAnswers.FILTER_SORT)
        )) {
