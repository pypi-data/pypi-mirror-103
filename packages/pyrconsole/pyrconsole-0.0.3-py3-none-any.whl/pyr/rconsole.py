...
class rconsole:
    ...
    def init(self):
        if (not fileio.exists(cons.diary)):
            ...
            cons.cc_dirs = s.cwd_("alogs/ccReplace/ccCtrlCCtrlVDirs.txt")
            if (s.is_linux()):
                cons.p = "/home"
            else:
                cons.p = "C:"
            
            filebackup_.backup_dir = s.cwd_("alogs/DirBackup")
            fileio.mkdir(filebackup_.backup_dir)
            filebackup_.files_dir = s.cwd_("alogs/DirBackup/files")
            fileio.mkdir(filebackup_.files_dir)
            filebackup_.paths_dir = s.cwd_("alogs/DirBackup/paths")
            fileio.mkdir(filebackup_.paths_dir)
