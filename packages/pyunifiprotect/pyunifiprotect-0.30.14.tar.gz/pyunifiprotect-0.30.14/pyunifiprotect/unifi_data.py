...
def event_from_ws_frames(state_machine, minimum_score, action_json, data_json):
    ...
    processed_event = process_event(event, minimum_score, LIVE_RING_FROM_WEBSOCKET, action)
...
def process_event(event, minimum_score, ring_interval, action):
    ...
    if action == "add" and not event.get("smartDetectEvents") and not event.get("smartDetectTypes"):
        _LOGGER.info("RESETTING OBJECT")
        processed_event["event_object"] = []
        
    if event_type in (EVENT_MOTION):
    ...
    elif event_type in (EVENT_SMART_DETECT_ZONE):
        processed_event["event_object"] = event.get("smartDetectTypes")
