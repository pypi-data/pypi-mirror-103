...
class GameTemplate(MonitoredClass):
    ...
    def draw_monitoredclass_stats(self, monitoredClass: MonitoredClass, surface: pygame.Surface, font: pygame.font.SysFont = None, offset_rectangle: Rectangle = None):
        if font is None:
        ...
        for key, val in tracked_time:
            offset_rectangle.y = y_off
            self.draw_text(f"{key}: {round(val, 2)} sec", surface, color=Color.LEMON_CHIFFON, offset_rect=offset_rectangle, font=font)
            y_off += font.get_height() + 3