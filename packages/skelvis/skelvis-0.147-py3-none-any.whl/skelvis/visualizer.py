...
class VideoPlayer:
    def __init__(self, plot: Plot, fps: int, frames: np.ndarray, video_player: Play = None):
        self.plot: Plot = plot
        self.video_player: Optional[Play] = video_player if video_player is not None else Play(
            value=0,
    ...
    def get_video_player_widget(self) -> HBox:
        frame_slider = IntSlider(value=0, min=0, max=self.frames.shape[0] - 1, step=1, description='Frame')
...
class LossContainer:
    def __init__(self, pred_skeletons: np.ndarray, gt_skeletons: np.ndarray,
        ...
        self.pred_skeletons: np.ndarray = pred_skeletons
        self.gt_skeletons: np.ndarray = gt_skeletons
        self.loss: Callable[[np.ndarray, np.ndarray], float] = loss

    def get_loss_tab(self) -> Tab:
        self.loss = lambda x, y: x + y
        return Tab(children=[Label(value='Hello')])
...
class SkeletonVisualizer:
    ...
    def visualize_with_ground_truths(
            ...
            automatic_camera_orientation: bool = False, loss: Callable[[np.ndarray, np.ndarray], float] = L2) -> None:
        ...
        loss_container: LossContainer = LossContainer(pred_skeletons, gt_skeletons, loss)
        self.visualize(
    ...
    def visualize_video(
            ...
            additional_tabs: List[Tuple[str, Widget]] = None) -> None:
        self.__assert_include_arguments(include_names, include_coordinates)
        ...
        self.plot.display()
        visibility_widget: HBox = HBox([self.joint_names_visible, self.joint_coordinates_visible])
        video_player: VideoPlayer = VideoPlayer(self.plot, fps, frames)
    ...
    def visualize_video_with_ground_truths(
        ...
        loss_container: LossContainer = LossContainer(pred_frames, gt_frames, loss)
        self.visualize_video(
            frames, colors, fps, include_names, include_coordinates, automatic_camera_orientation, is_gt_list,
            additional_tabs=[('Losses', loss_container.get_loss_tab())])
