...
class LossContainer:
    def __init__(self, pred_skeletons: np.ndarray, gt_skeletons: np.ndarray, joint_set: JointSet,
        ...
        self.skeleton_loss_labels: List[Label] = [Label(value='test')] * self.number_of_skeletons
    ...
    def __create_empty_loss_tabs(self):
        ...
        for i in range(self.number_of_skeletons):
            joint_loss_labels: List[Label] = [Label(value='test')] * self.joint_set.number_of_joints
            ...
            skeleton_loss_tabs.append(HBox(children=[joint_names, joint_losses, skeleton_labels, skeleton_values]))
        ...
        if self.is_video:
            ...
            statistics_values_column.append(Label(value=str(self.min_loss_frame_index + 1)))
            statistics_values_column.append(Label(value=str(self.max_loss_frame_index + 1)))
    ...
    def __set_loss_labels(self, index: int) -> None:
        for i in range(self.number_of_skeletons):
            ...
            self.min_joint_loss_name_labels[i].value = self.joint_set.names[self.min_skeleton_loss_indices[i][index]]
            self.max_joint_loss_name_labels[i].value = self.joint_set.names[self.max_skeleton_loss_indices[i][index]]
        self.all_loss_label.value = '{:.3f}'.format(self.all_losses[index])
        self.min_loss_index_label.value = self.min_skeleton_loss_indices[index]
        self.max_loss_index_label.value = self.max_skeleton_loss_indices[index]
