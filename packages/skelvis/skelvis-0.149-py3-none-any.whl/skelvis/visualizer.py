...
class LossContainer:
    ...
    def get_loss_tab(self) -> Tab:
        ...
        # TODO: fill out losses, link player value to loss indices
        return loss_tab

    def __create_empty_loss_tabs(self):
        skeleton_loss_tabs: List[HBox] = []
        skeleton_losses_column: List[HBox] = []
        for i in range(self.number_of_skeletons):
            skeleton_label: Label = Label(value='Skeleton {:d} loss:'.format(i + 1))
            skeleton_loss: Label = Label(value='test')
            self.skeleton_loss_labels.append(skeleton_loss)
            skeleton_losses_column.append(HBox(children=[skeleton_label, skeleton_loss]))
            joint_loss_labels: List[Label] = []
            joint_losses_column: List[HBox] = []
            for j in range(self.joint_set.number_of_joints):
                joint_label: Label = Label(value=self.joint_set.names[j])
                joint_loss: Label = Label(value='test')
                joint_loss_labels.append(joint_loss)
                joint_losses_column.append(HBox(children=[joint_label, joint_label]))
            joint_losses: VBox = VBox(children=joint_losses_column)
            self.joint_loss_labels.append(joint_loss_labels)
            skeleton_statistics_column: List[HBox] = []
            all_joint_loss_label: Label = Label(value='test')
            self.all_joint_loss_labels.append(all_joint_loss_label)
            skeleton_statistics_column.append(HBox(children=[Label(value='All losses:'), all_joint_loss_label]))
            min_loss_joint_label: Label = Label(value='test')
            self.min_joint_loss_name_labels.append(min_loss_joint_label)
            skeleton_statistics_column.append(HBox(children=[Label(value='Joint with min loss:'), min_loss_joint_label]))
            max_loss_joint_label: Label = Label(value='test')
            self.max_joint_loss_name_labels.append(max_loss_joint_label)
            skeleton_statistics_column.append(HBox(children=[Label(value='Joint with max loss:'), max_loss_joint_label]))
            skeleton_statistics: VBox = VBox(children=skeleton_statistics_column)
            skeleton_loss_tabs.append(HBox(children=[joint_losses, skeleton_statistics]))
        skeleton_losses: VBox = VBox(children=skeleton_losses_column)
        statistics_column: List[HBox] = [
            HBox(children=[Label(value='All losses:'), self.all_loss_label]),
            HBox(children=[Label(value='Min loss skeleton index:'), self.min_loss_index_label]),
            HBox(children=[Label(value='Min loss skeleton index:'), self.max_loss_index_label])
        ]
        if self.is_video:
            self.min_loss_frame_index: int = np.argmin(self.skeleton_losses, axis=-1)
            min_loss_jump_button: Button = Button(description='Jump to frame with min loss')
            min_loss_jump_button.on_click(self.__jump_to_min_loss_frame)
            self.max_loss_frame_index: int = np.argmax(self.skeleton_losses, axis=-1)
            max_loss_jump_button: Button = Button(description='Jump to frame with max loss')
            max_loss_jump_button.on_click(self.__jump_to_max_loss_frame)
            statistics_column.append(HBox(children=[Label(value='Min loss frame index:'),
                                                    Label(value=str(self.min_loss_frame_index + 1)),
                                                    min_loss_jump_button]))
            statistics_column.append(HBox(children=[Label(value='Max loss frame index:'),
                                                    Label(value=str(self.max_loss_frame_index + 1)),
                                                    max_loss_jump_button]))
        statistics: VBox = VBox(children=statistics_column)
        loss_tabs: List[HBox] = [HBox(children=[skeleton_losses, statistics])] + skeleton_loss_tabs
