...
class _DatastoreClient:
    ...
    def resample(self, datastore_id: UUID) -> Union[DtlError, Job]:
        """
        Trigger a resampling job for the datastore with the given datastore id.
        
        :param datastore_id: id of the datastore to resample
        :return: a datalogue job that is resampling the datastore
        """
        res = self.http_client.make_authed_request(
            self.service_uri + "/datastores/" + str(datastore_id) + "/refresh-samples", HttpMethod.PUT
        )
        if isinstance(res, DtlError):
            return res
