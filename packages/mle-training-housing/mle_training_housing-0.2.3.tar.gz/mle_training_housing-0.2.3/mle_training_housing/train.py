...
def main():
    ...
    *(Folder is an optional parameter. Default value is 'housing_run_v1'.)*
    ...
    # housing = load_housing_data(housing_path_final)
        ...
        raise FileNotFoundError("housing.csv is not present in the " + housing_path_final + ". Please create and try again.")
        sys.exit(1)
    ...
    housing = pd.concat(housing, pd.DataFrame(housing_extra_attribs, index=housing.index, columns=["rooms_per_household", "population_per_household", "bedrooms_per_room"]), axis=1)
    ...
    numeric_transformer = Pipeline(steps=[("imputer", SimpleImputer(strategy="median")), ("attribs_adder", CombinedAttributesAdder())])
