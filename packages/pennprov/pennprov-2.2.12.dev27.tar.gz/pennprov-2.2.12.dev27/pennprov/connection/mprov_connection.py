...
class MProvConnection:
    ...
    def store_activity(self,
        ...
        activity_id = self.get_activity_id('activity', location)
        tok = pennprov.ProvTokenModel(token_value=self.get_token_hash(activity_id))
    ...
    @classmethod
    def get_token_hash(cls, token):
        # types: (str) -> str
        """
        Returns a string created by hashing the given token. Used for cases where the length of
    ...
    def get_token_qname(cls, token):
        ...
        :return:
        """
        return cls._get_qname(cls.get_token_hash(token))
