...
class CLRunner(object) :
    ...
    def param(**kwargs) :
        def result(method) :
            param = CLRunner._normalize_param( kwargs, kwargs['name'] if 'name' in kwargs else method.__name__ )
            if param['doc'] is None :
                param['doc'] = method.__doc__
    ...
    def command(name=None,params=None,aliases=None,doc=None) :
        def result(method) :
            command_name = method.__name__ if name is None else name
            command_doc = method.__doc__ if doc is None else doc
            method._cl_command = {
                'name' : command_name,
            ...
            for attrname in dir(command_line_runnable) :
                ...
                if hasattr(item,'_cl_command') :
                    command = {
                        ...
                        'doc' : item._cl_command['doc'] if item._cl_command['doc'] is not None else item.__doc__,
                        'code' : item,
                        }
                    delattr(item,'_cl_command')
                    for alias in command['aliases'] :
                        commands[alias] = command
                if hasattr(item,'_cl_param') :
                    param = item._cl_param
                    param['code'] = item
                    delattr(item,'_cl_param')
