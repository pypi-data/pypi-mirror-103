...
from ._util._utils import _camel_to_snake_case
...
class RunSettings(WorkspaceTelemetryMixin):
    ...
    def _set_parameter(self, key, value):
        if key in self._params_mapping.keys():
            self._handle_params_mapping_keys(key, value)
        else:
            self._handle_sections_mapping_keys(key, value)

    def _handle_params_mapping_keys(self, key, value):
        super().__setattr__(key, value)
    ...
    def _handle_sections_mapping_keys(self, key, value):
        section = self._sections_mapping[key]
        section._clear_section_values()
        if isinstance(value, dict):
            for k in value.keys():
                section.__setattr__(k, value[k])
        elif key == "early_termination":
            from azureml.train.hyperdrive import BanditPolicy, MedianStoppingPolicy, TruncationSelectionPolicy, \
                NoTerminationPolicy
            if not isinstance(value, (BanditPolicy, MedianStoppingPolicy, TruncationSelectionPolicy,
                                      NoTerminationPolicy)):
                raise UserErrorException(
                    "'{}' is invalid for 'early_termination', expected type: BanditPolicy, MedianStoppingPolicy, "
                    "TruncationSelectionPolicy, NoTerminationPolicy.".format(value))
            dict_v = value.to_json()
            # dict_v is like: {'name': 'Bandit', 'properties': {'evaluation_interval': 1, 'delay_evaluation': 5,
            # 'slack_amount': 5}}
            self._store.set_value(section._params_mapping['policy_type'].id, _camel_to_snake_case(dict_v['name']))
            if dict_v['name'] != "Default":
                # NoTerminationPolicy, Default policy doesn't have 'properties'
                for k, v in dict_v['properties'].items():
                    if k != "exclude_finished_jobs":
                        self._store.set_value(section._params_mapping[k].id, v)
        else:
            raise UserErrorException("'{0}' is invalid for runsetting '{1}'.".format(value, key))
