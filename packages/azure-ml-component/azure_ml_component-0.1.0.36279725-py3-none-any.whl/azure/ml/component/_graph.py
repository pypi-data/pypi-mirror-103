...
from ._util._utils import _value_type_json_serializable, _ensure_dataset_saved_in_workspace
...
class _GraphEntityBuilder(object):
    ...
    def _produce_module_runsettings(self, graph_entity: GraphDraftEntity, module: Component,
        ...
        if module.runsettings is None:
            return None
        # Prepare required pipeline parameter, add them to graph.
        _GraphEntityBuilder._append_component_runsetting_pipeline_parameter_to_interface(
            graph_entity, pipeline_parameters, module)
    ...
    @staticmethod
    def _append_component_runsetting_pipeline_parameter_to_interface(graph_entity, pipeline_parameters, component):
        ...
        def append_by_type(obj):
            if isinstance(obj, PipelineParameter):
                _GraphEntityBuilder._append_pipeline_parameter_to_interface(
                    graph_entity, pipeline_parameters, obj.name)
            elif isinstance(obj, _ParameterAssignment):
                _GraphEntityBuilder._append_pipeline_parameter_in_assignment_to_interface(
                    graph_entity, pipeline_parameters, obj)
            elif isinstance(obj, dict):
                for dict_value in obj.values():
                    append_by_type(dict_value)
            elif type(obj) in [tuple, list]:
                for list_value in obj:
                    append_by_type(list_value)

        for value in runsettings_values.values():
            append_by_type(value)
    ...
    def _update_module_node_params(
        ...
        for output in module.outputs.values():
            ...
            # if module's output is pipeline output, build as data path assignment
    ...
    def _resolve_runsetting_parameter_assignment(runsetting_def, runsetting_value,
        ...
        if isinstance(runsetting_value, Input):
            runsetting_value = runsetting_value._get_internal_data_source()
        if isinstance(runsetting_value, _ParameterAssignment):
            value_type = _GraphEntityBuilder.CONCATENATE
            # Get the resolved value if is _ParameterAssignment
            resolved_value = runsetting_value.value
            assignments = _GraphEntityBuilder._get_assignments_to_concatenate(runsetting_value, pipeline_parameters)
        elif isinstance(runsetting_value, PipelineParameter):
            if runsetting_value.name in pipeline_parameters:
                value_type = _GraphEntityBuilder.GRAPH_PARAMETER_NAME
                # Link the pipeline parameter name if is pipeline parameter
                resolved_value = runsetting_value.name
            else:
                resolved_value = runsetting_value.default_value
        ...
        if is_compute_target:
            ...
            return RunSettingParameterAssignment(
                ...
                assignments_to_concatenate=assignments)
        else:
            should_json_dumps = (resolved_value is not None and
                                 runsetting_def.parameter_type == RunSettingParameterType.json_string and
            ...
            if should_json_dumps:
                ...
                if not _value_type_json_serializable(resolved_value):
                    raise UserErrorException(
                        f"Parameter '{runsetting_def.name}' on component '{component._graph_path}'"
                        " is invalid, which is not JSON serializable.")
            ...
            return RunSettingParameterAssignment(name=runsetting_def.name, value=resolved_value, value_type=value_type,
                                                 assignments_to_concatenate=assignments)
...
def _parse_parameter_expressions_for_sweep_search_space(parameter, parameter_expression):
    """
    ...
    """
    def check_if_conditional_hyperparameter(values):
        for model in values:
    ...
    if type == 'choice':
        might_conditional_hyperparameter = values[0]
        if check_if_conditional_hyperparameter(might_conditional_hyperparameter):
            dict_v = {'type': type, 'values': might_conditional_hyperparameter}
            for model in dict_v['values']:
                for k in model:
                    if isinstance(model[k], list):
                        dict_k = _parse_parameter_expressions_for_sweep_search_space(k, model[k])
                        model[k] = dict_k
        else:
            [options] = values
...
def _populate_submission_runsettings(component, pipeline_parameters=None):
    ...
    for name, p in runsettings_definition.params.items():
        runsetting_param = _GraphEntityBuilder._resolve_runsetting_parameter_assignment(
            p, runsettings_values.get(p.id), component, pipeline_parameters)
        ...
        for p in runsettings_definition.compute_params.values():
            compute_settings.append(
                _GraphEntityBuilder._resolve_runsetting_parameter_assignment(
                    p, compute_runsettings_values.get(p.id), component, pipeline_parameters))
    ...
    if algorithm is not None:
        ...
        for p in linked_params:
            if input_values[p] is None:
                input_values[p] = {}
            # This section is used for parsing input contract hyperparameter expressions from azureml.train.hyperdrive
            # Logic & assumption: for now, we don't support/have list type input, therefore, we use
            # isinstance(input, list) to predicate that the input is a sweep-contract-hyperparameter-contract
            # expression. Attention, if we support list-type parameters, this logic must be changed.
            elif isinstance(input_values[p], list):
                dict_v = _parse_parameter_expressions_for_sweep_search_space(parameter=p,
                                                                             parameter_expression=input_values[p])
                input_values[p] = dict_v
            ...
            elif not isinstance(input_values[p], dict):
                logging.warning("Parameter '%s' type mismatch. Required 'dict', " % p +
                                "got '%s'. Will use default value." % type(input_values[p]).__name__)
                input_values[p] = {}
