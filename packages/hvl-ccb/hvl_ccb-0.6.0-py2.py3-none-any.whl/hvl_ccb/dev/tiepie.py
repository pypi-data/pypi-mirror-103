...
class TiePieOscilloscope(SingleCommDevice):
    ...
    def list_devices() -> ltp.devicelist.DeviceList:
        ...
        if device_list:
            ...
            for item in ltp.device_list:
                ...
                logging.info(f"  Serial number:     {item.serial_number}")
                logging.info(f"  Available types:   {ltp.device_type_str(item.types)}")
        else:
    ...
    def is_triggered(self) -> bool:
        ...
        logging.debug(f"TiePie has triggered: {_is_triggered}")
    ...
    def is_measurement_data_ready(self) -> bool:
        ...
        logging.debug(
            f"TiePie has measurement data ready: {_is_measurement_data_ready}"
        )
    ...
    def force_trigger(self) -> None:
        ...
        logging.info("A force trigger was sent to TiePie")
    ...
    def _check_record_length(self, data_array: np.ndarray) -> None:
        ...
        if self.config_osc is None:
            logging.warning("Oscilloscope is not configured")
            return None
