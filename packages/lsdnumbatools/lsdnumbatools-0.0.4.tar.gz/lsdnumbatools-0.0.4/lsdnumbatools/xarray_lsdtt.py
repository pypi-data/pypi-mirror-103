...
class DEM(object):
  ...
  def load_main_raster(self, fname = "dem.tif", nodata_method = "closed"):
    ...
    self.ds = xr.Dataset()
    self.ds["y"] = np.arange(self.graph.ds["y_min"].item(0)+self.graph.ds["dy"].item(0)/2,self.graph.ds["y_max"].item(0),self.graph.ds["dy"].item(0))[::-1]
    self.ds["x"] = np.arange(self.graph.ds["x_min"].item(0)+self.graph.ds["dx"].item(0)/2,self.graph.ds["x_max"].item(0),self.graph.ds["dx"].item(0))
    self.ds["topography"] = (("y","x"), array)
