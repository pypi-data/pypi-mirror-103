# -!- coding: utf-8 -!-
import re, os, requests, time, random, chardet
...
class req_base():
    ...
    def __init__(self):
        ...
        self.base_headers = deepcopy(self.base_headers)
    ...
    def delete_ua(self):
        ...
        self.base_headers.pop('User-Agent', '')
    def update_ua(self):
        ua = self.get_ua()
        if ua:
    ...
    def r_get_encoding(self, r): return chardet.detect(r.content)['encoding']

    def r_get_html(self, r):
        content = r.content
        # 优先展示中文
        for x in ('utf8', 'gbk', r.encoding):
            try: return content.decode(x)
            except: pass
        # 无法展示中文时, 才展示非中文
        try: return r.text
        except: return content.decode(self.r_get_encoding(r))
    ...
    def url_get_title(self, url):
        r = self.get(url=url, headers=self.base_headers)
