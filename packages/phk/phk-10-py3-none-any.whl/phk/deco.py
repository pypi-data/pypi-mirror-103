# -!- coding: utf-8 -!-
import re, os, time, random, asyncio
...
def 异步简用(func):
    def newfunc(*vs, **kvs):
        async def main(*vs, **kvs):
            return await func(*vs, **kvs)
        return asyncio.run(main(*vs, **kvs))
    return newfunc


def 失败重试(最高次数, 重试间隔=0, 每次失败执行=None):
    def retry(func):
        def newfunc(*vs, **kvs):
            for i in range(1, 最高次数+1):
                try: return True, func(*vs, **kvs)
