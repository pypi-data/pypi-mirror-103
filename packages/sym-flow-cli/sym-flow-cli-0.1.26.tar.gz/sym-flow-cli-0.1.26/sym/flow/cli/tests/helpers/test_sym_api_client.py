...
from sym.flow.cli.errors import NotAuthorizedError, SymAPIRequestError, SymAPIUnknownError
from sym.flow.cli.helpers.sym_api_client import SymAPIClient
from sym.flow.cli.helpers.sym_api_service import ConnectorType
...
@pytest.fixture
def sym_api_client(sandbox) -> Iterator[SymAPIClient]:
...
class TestSymAPIClient:
    ...
    def test_get_connectors(self, mock_api_get, sym_api_client):
        data, request_id = sym_api_client.get_connectors(ConnectorType.SLACK.value)
        assert request_id == "test-request-id"
    ...
    def test_get_slack_install_url(self, mock_api_get, sym_api_client):
        url, request_id = sym_api_client.get_slack_install_url()
    ...
    def test_uninstall_slack(self, mock_api_get, sym_api_client):
        assert (
            sym_api_client.uninstall_slack(workspace_id="T1234567") == "test-request-id"
        )
        mock_api_get.assert_called_once_with(
