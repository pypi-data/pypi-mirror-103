...
if os.path.isdir('/data/data/com.termux/files/usr'):
    tmp_dir = os.path.join('/data/data/com.termux/files/usr/tmp', 'tblock')
    lib_dir = os.path.join('/data/data/com.termux/files/usr/lib', 'tblock')
elif os.path.isdir('/var/lib/tblock/') and os.path.isdir('/tmp/tblock/'):
    lib_dir = '/var/lib/tblock/'
    tmp_dir = '/tmp/tblock/'
else:
    sys.exit(
        'Error: TBlock does not seem to support your OS.\n'
        'If this seems to be a bug, you can report it at: https://codeberg.org/tblock/tblock/issues'
    )
