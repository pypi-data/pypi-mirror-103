...
class Rule:
    ...
    def rules_count(db: sqlite3.Connection) -> dict:
        ...
        """
        return {
            "total": len(Rule.fetch_all_rules(db).keys()),
            "allow": len(db.cursor().execute('SELECT "domain" FROM "rules" WHERE policy="allow";').fetchall()),
            "block": len(db.cursor().execute('SELECT "domain" FROM "rules" WHERE policy="block";').fetchall()),
            "redirect": len(db.cursor().execute('SELECT "domain" FROM "rules" WHERE policy="redirect";').fetchall()),
            "user": len(db.cursor().execute('SELECT "domain" FROM "rules" WHERE priority=1;').fetchall())
        }
