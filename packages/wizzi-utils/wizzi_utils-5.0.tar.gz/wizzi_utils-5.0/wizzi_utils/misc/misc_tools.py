...
def get_timer_delta(start_timer: float, with_ms: bool = False) -> datetime.timedelta:
    """
    :param start_timer: begin time
    :param with_ms: if microseconds neened - set to true. else: lowest is seconds
    :return:
    time = get_timer()
...
def get_base_file_name(depth: int = 1) -> str:
...
def get_function_name_and_line(depth: int = 1) -> str:
    # +1 because of this function
    ret_val = '{} line {}'.format(get_base_file_name(depth + 1), get_line_number(depth + 1))
    return ret_val


def get_base_file_and_function_name(depth: int = 1) -> str:
    # +1 because of this function
    ret_val = '{}::{}'.format(get_base_file_name(depth + 1), get_function_name(depth + 1))
    return ret_val
