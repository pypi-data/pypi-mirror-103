...
class Crp:
    ...
    def getLastOrderparams(self,matrix:pd.DataFrame,train_df:pd.DataFrame):
        """
        Params:
            matrix      ->
            train_df    ->
        ...
        target = train_df[train_df.customerid.map(matrix['createdat']) == train_df.order_date][['customerid',
    ...
    def getTotalSpentLMonth(self,train_df:pd.DataFrame,dateFlag_sep,df_matrix:pd.DataFrame):
        """
        Params:
    ...
    def getFeaturesMatrix(self,train_df,org_df,timeFlag,items_dataset,customer_name,train_window,sq_pass):
        """
        Params:
        ...
        output = pd.merge(df_matrix,orders_param,on='customerid',how='left')
    ...
    def prepareDF(self,df,dateFlag_sep,date_flag_min,lag,items_dataset,customer_name,train_window,sq_pass):
        ...
        # Calculate features
        df = self.getFeaturesMatrix(train_data,original_dataset,lag,items_dataset,customer_name,train_window,sq_pass)
    ...
    def buildDF(self,original_dataset:pd.DataFrame,lag:int,train_window:int,customer_name,sq_pass,items_dataset:pd.DataFrame = None):
        """
        Params:
        ...
        output.append(self.prepareDF(df,dateFlag_sep,date_flag_min,lag,items_dataset,customer_name,train_window,sq_pass))
