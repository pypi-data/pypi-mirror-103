...
@LastEditTime: 2021-04-23 14:04:52
...
class NewCoinExchangeHandler(ClientBaseHandler):
    ...
    def get_async(self):
        ...
        if coin_exchange.goods_type == 1:
            ...
            days_prop_count = days_prop_count.decode() if days_prop_count else 0
            if int(days_prop_count) >= coin_exchange.day_limit:
                return self.client_reponse_json_error("Error", "对不起，每日兑换已达上限")
        ...
        if update_result:
            try:
                ...
                if coin_exchange.goods_type == 1:
                    ...
                    prop_log.history_value = user_dict["surplus_coin"]
