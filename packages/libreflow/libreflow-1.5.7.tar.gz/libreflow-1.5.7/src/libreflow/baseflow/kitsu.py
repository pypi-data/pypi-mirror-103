...
class KitsuAPIWrapper(flow.Object):
    ...
    def upload_preview(self, kitsu_entity, task_type_name, task_status_name, file_path, comment="", user_name=None):
        ...
        # Check if uploading user is assigned to the task
        if not self.user_is_assigned(user, task):
            self.root().session().log_error((
                f"User '{user_name}' is not assigned to task '{task_type_name}' "
                "of entity '%s'." % kitsu_entity['name']
            ))
            return False
