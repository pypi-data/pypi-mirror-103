...
class UploadPlayblastToKitsu(flow.Action):
    ...
    def get_buttons(self):
        ...
        if self._kitsu_entity is None or task_type is None:
            self.message.set((
        ...
        task = kitsu_api.get_task(self._kitsu_entity, task_type)
        
        if user is None or task is None or not kitsu_api.user_is_assigned(user, task):
            self.message.set((
        ...
        self.message.set("<h2>Upload playblast to Kitsu</h2>")
    ...
    def run(self, button):
        ...
        kitsu_api.upload_preview(
            kitsu_entity=self._kitsu_entity,
