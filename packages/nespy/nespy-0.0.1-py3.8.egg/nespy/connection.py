...
class Connection:
    ...
    def execute_query(self, query, strategy):
        ...
        ----------
        query : str
                the query itself as a string. The query has to follow NebulaStream's Query API
        strategy : str
    ...
    def get_queries_with_status(self, status):
        ...
        Notes
        -------
        (Took from the comments of NES:
        https://github.com/nebulastream/nebulastream/blob/master/include/Catalogs/QueryCatalogEntry.hpp)
        ...
        if status.casefold() in status_list:
            ...
            if status_response.status_code == 200:
                if type(status_response.json(), list):
                    return status_response.json()
                elif type(status_response.json(), None):
                    return []
    ...
    def get_logical_stream(self, position_or_name):
        ...
        position_or_name : str or int
                   position in dictionary or name of logical stream. Name has to be writting correctly or else it does not work
    ...
    def add_logical_stream(self, logical_stream_name, schema):
        ...
        schema : str
                 the schema of the logical stream. Has to be in C++ at the moment
