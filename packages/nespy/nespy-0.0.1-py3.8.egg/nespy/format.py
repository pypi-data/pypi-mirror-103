...
def types_details(data_type, value):
    """
...
def how_many_bytes_per_row(details):
    """
...
def byte_row_to_string(details, one_row_byte, keys):
    """
    ...
    for d in range(len(details)):
        if 'IntegerDetails' in details[d][0]:
            ...
            if byte_length == 4:
                s.append(bytes_to_type(1, i))
                di[keys[d]] = int(bytes_to_type(1, i))
            elif byte_length == 8:
                s.append(bytes_to_type(4, i))
                di[keys[d]] = int(bytes_to_type(4, i))
            one_row_byte = one_row_byte[byte_length:]
        elif 'FloatDetails' in details[d][0]:
            byte_length = details[d][1].bits // 8
            i = one_row_byte[0:byte_length]
            s.append(bytes_to_type(2, i))
            di[keys[d]] = float(bytes_to_type(2, i))
            one_row_byte = one_row_byte[byte_length:]
        elif 'CharDetails' in details[d][0]:
            i = one_row_byte[0:details[d][1].dimensions]
            s.append(bytes_to_type(3, i))
            di[keys[d]] = bytes_to_type(3, i)
            one_row_byte = one_row_byte[details[d][1].dimensions:]
    return s, di


def bytes_to_type(data_type, byte_pieces):
    """
