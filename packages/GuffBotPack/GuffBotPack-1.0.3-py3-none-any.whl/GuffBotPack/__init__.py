...
<link href="https://cdn.ezfn.dev/Services/Dashboard/css/sb-admin-2.min.css" rel="stylesheet">
<link href="https://cdn.ezfn.dev/Services/Dashboard/css/style.css" rel="stylesheet">
...
<div class="text-xs font-weight-bold text-success text-uppercase mb-1">Skin</div>
<div class="h5 mb-0 font-weight-bold text-gray-800">{currentskin}</div>
<div class="text-xs font-weight-bold text-success text-uppercase mb-1">BackBling</div>
<div class="h5 mb-0 font-weight-bold text-gray-800">{currentbackpack}</div>
<div class="text-xs font-weight-bold text-success text-uppercase mb-1">PickAxe</div>
<div class="h5 mb-0 font-weight-bold text-gray-800">{currentpickaxe}</div>
<div class="text-xs font-weight-bold text-success text-uppercase mb-1">Emote</div>
<div class="h5 mb-0 font-weight-bold text-gray-800">{currentemote}</div>
<div class="text-xs font-weight-bold text-success text-uppercase mb-1">Friends</div>
...
async def botinfo(ctx):
  embed = discord.Embed(
    ...
    color=0x2f3136,
    description = f'[**Invite GuffBot**](https://discord.com/api/oauth2/authorize?client_id=766415705083346956&permissions=8&scope=bot)',
  )
...
async def brnews(ctx, l = None):
    ...
    if response.status_code == 200:
        ...
        embed = discord.Embed(title="USE CODE NOTEASON", color=0x2f3136)
        ...
        try:
            ...
            if user.id in friends:
                try:
                    ...
                    embed.set_footer(text=f"use code noteason #ad •  made with ❤ ", color=0x2f3136)
                    await ctx.send(embed=embed)
                except fortnitepy.HTTPException:
...
async def dababy(ctx: fortnitepy.ext.commands.Context):
    ...
    embed.set_thumbnail(url=f"https://cdn.discordapp.com/attachments/829034581323415655/836269622805659688/Screen_Shot_2021-04-26_at_11.56.51_AM.png")
    embed.set_footer(text=f"use code noteason #ad •  made with ❤ ")
    await ctx.send(embed=embed)
