...
class ANN():
    ...
    def __init__(self, input_shape, structure, output, activation= "sigmoid", loss = "auto", initializer = "auto", strict=False, delta=1e-7):
        ...
        if self.initializer == "uniform":
            ...
            for i in range(len(structure)):
                if i == 0:
                    self.w_layers.append(np.random.randn(input_shape[1], structure[0]) / 2*np.sqrt(input_shape[1]*structure[0]))
                else:
                    self.w_layers.append(np.random.randn(structure[i-1], structure[i]) / 2*np.sqrt(structure[i-1]*structure[i]))
