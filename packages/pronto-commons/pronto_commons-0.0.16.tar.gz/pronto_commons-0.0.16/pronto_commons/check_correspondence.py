...
class CorrespondenceChecker:
    ...
    def check(
        ...
        for field in fields(entity):
            ...
            elif field.type is ObjectId:
                expect = ObjectIdField, "ObjectId"
            else:
                try:
                    expect = self.entity_to_schema_and_swagger[field.type]
                except KeyError:
                    assert (
                        False
                    ), f"entity {entity}: unknown type {field.type} encountered for field {field.name}. types that can be wrapped in Optional[...] should be specified above"
            ...
            if swagger_type:
                ...
                for o in self.override_expected_types:
                    if (
                        ...
                        and field.type is o.entity_type_to_match
                    ):
                    ...
                    ), f"entity {entity} field {field.name} has ref in swagger {properties[field.name]['$ref']} but it should be {expect[1]} according to {field.type}"
