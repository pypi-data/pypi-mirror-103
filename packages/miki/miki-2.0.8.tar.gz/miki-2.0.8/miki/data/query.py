...
class Query(object):
	...
	def get_dataframe(self, year, field, unit, update):
		...
		return df
	def get_stock(self, security, start, end, field_list, unit, fq=None, use_cache=False):
		# 获取单只股票数据 use_cache会在内存缓存数据
		start,end = pd.to_datetime(pd.to_datetime(start).date()),pd.to_datetime(end)
		if use_cache and security in self.cache_data and end.date()==self.cache_date:
			df1,df2 = self.cache_data[security][0].copy(), self.cache_data[security][1].copy()
		else:
			...
			ctable1 = ctable1.fetchwhere('(date<={})&(date>={})'.format(end.timestamp(), start.timestamp()), outcols=outcols)
			ctable2 = bcolz.open(path1d, mode='r')
			outcols = ['date'] + [i for i in field_list if i in ['high_limit','low_limit']]
			ctable2 = ctable2.fetchwhere('(date<={})&(date>={})'.format(end.timestamp(), start.timestamp()), outcols=outcols)
			df1 = ctable1.todataframe()
			df2 = ctable2.todataframe()
			if use_cache:
				self.cache_date = end.date()
				self.cache_data[security] = [df1.copy(), df2.copy()]
		local_timestamp = bcolz.open(DataFunction.get_path('000001.XSHG', unit='1d')+'/date', mode='r')[-1]
		# 当天数据
		if end.timestamp()>local_timestamp:
			DataBcolz.get_today_data()
			today_data, security_list, _field_list = dataGlovar.today_data
			if len(today_data)>0 and today_data[0,0,0]>local_timestamp and security in security_list:
				id1,id2 = security_list.index(security), [_field_list.index(i) for i in df1.columns]
				today_df = pd.DataFrame(today_data[:,id1,id2], columns=df1.columns)
				df1 = pd.concat([df1, today_df], axis=0)
