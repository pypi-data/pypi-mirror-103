...
class SparkManage(object):
    ...
    def __init__(self, ctx, code_path, is_async=False, is_log=False,
        ...
        if self.is_debug:  # mac本地测试环境
            ...
            oss_config_file = '/opt/djcelery44/djcelery44/scripts/tools/spark_s3.conf'
    ...
    def get_submit_spark_cmd(self):
        ...
        """
        base_cmd = f"""KUBECONFIG=~/.kube/config ./bin/spark-submit \
...
if __name__ == "__main__":
    sm = SparkManage(
        ...
        task_params={'config': {'test': '哈哈', 'a': '测试参数'}, 'config1': '测试'},
        is_debug=True)
    sm.upload_code()
