...
def main():
    ...
    parser.add_argument("--debug_cp", help="Copy files before deleting tmp dir to given directory for debugging")
    parser.add_argument("--template", help="The path to an alternative template")
    parser.add_argument("input", nargs="+", help="Input to generate documentation from. Can either be 1 or more directories containing the specified structure or an appropriate JSON")
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            if os.path.isdir(args.input[0]):
                ...
                jsonFilePath = args.input[0]
            
            if not args.output:
                if args.input[0].endswith("/"):
                    outputFilePath = os.path.basename(args.input[0][:-1]) + "." + args.output_type.lower()
                else:
                    outputFilePath = os.path.basename(args.input[0]) + "." + args.output_type.lower()
            else:
            ...
            if args.debug_cp:
                shutil.copytree(tmpdir, os.path.join(args.debug_cp, "tmpdir"))
