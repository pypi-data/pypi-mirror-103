...
def get_dict_from_file_system(rootFolderPaths):
    resultDict = {}
    
    if isinstance(rootFolderPaths, (str, bytes)):
        folderList = [rootFolderPaths]
    else:
        folderList = rootFolderPaths
    
    for fPath in folderList:
        for aDir in os.listdir(fPath):
            dirp = os.path.join(fPath, aDir)
            if os.path.isdir(dirp):
                resultDict[aDir] = _parse_test_dir(dirp)
        logger.debug(f"Created json hierarchy for input folder {fPath}")
    return resultDict

def generate_json_from_file_system(rootFolderPaths, jsonOutputPath):
    with open(jsonOutputPath, "w") as fh:
        json.dump(get_dict_from_file_system(rootFolderPaths), fh, indent=4, sort_keys=True)
