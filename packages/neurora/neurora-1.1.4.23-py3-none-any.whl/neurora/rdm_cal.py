...
def eegRDM(EEG_data, sub_opt=1, chl_opt=0, time_opt=0, time_win=5, time_step=5, method="correlation", abs=False):
    ...
    if time_opt == 1:
        ...
        if chl_opt == 1:
            ...
            # initialize the RDMs
            rdms = np.zeros([subs, chls, ts], dtype=np.float64)
                    ...
                    for i in range(cons):
                        for j in range(cons):
                            ...
                            if (np.isnan(data[:, x, y, z, i]).any() == False) and \
                                    (np.isnan(data[:, x, y, z, j]).any() is False):
                                if method is 'correlation':
                                    # calculate the Pearson Coefficient
        ...
        for j in range(ny):
            for k in range(nz):
                ...
                if (mask_data[i, j, k] != 0) and (math.isnan(mask_data[i, j, k]) is False)\
                        and (np.isnan(fmri_data[:, :, :, j, k]).any() is False):
                    n = n + 1
    ...
    for sub in range(nsubs):
        for i in range(ncons):
            for j in range(ncons):
                ...
                if (np.isnan(data[:, i]).any() is False) and (np.isnan(data[:, j]).any() is False):
                    if method is 'correlation':
                        # calculate the Pearson Coefficient
