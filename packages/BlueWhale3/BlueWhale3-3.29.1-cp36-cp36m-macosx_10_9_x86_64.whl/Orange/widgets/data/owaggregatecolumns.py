...
class OWAggregateColumns(widget.OWWidget):
    ...
    operation = Setting(__("checkout.sum"))
    var_name = Setting("agg")
    auto_apply = Setting(True)

    Operations = {__("checkout.sum"): np.nansum, __("checkout.product"): np.nanprod,
                  __("checkout.min"): np.nanmin, __("checkout.max"): np.nanmax,
                  __("checkout.mean"): np.nanmean, __("checkout.variance"): np.nanvar,
                  __("checkout.median"): np.nanmedian}
    TimePreserving = (__("checkout.min"), __("checkout.max"), __("checkout.mean"), __("checkout.median"))
    ...
    def __init__(self):
        ...
        combo = gui.comboBox(
            box, self, "operation",
            label=__("label.operator"), orientation=Qt.Horizontal,
