...
class SunburstDiagram(EnergyPlotting):
    ...
    def create_parameters(self) -> dict:
        ...
        # Grid (Charging Energy) as first parent
        labels.append(categories[self.COMPONENTS_CH][self.GRID])
        ...
        values.append(0 + data.ac_pe_charging_energy - data.dc_power_charging_energy +
                      ...
                      data.storage_technology_loss_energy)
        ...
        labels_length_change = len(labels) - labels_length
        parents.extend(labels_length_change * [categories[self.COMPONENTS_CH][self.GRID]])

        # Losses (Charging) as parents of component-wise charging losses
        labels_length = len(labels)
        ...
        values.append(data.storage_technology_loss_energy)

        labels_length_change = len(labels) - labels_length
        parents.extend(labels_length_change * [self.LOSSES_CH])

        # Losses (Discharging) as parents of component-wise discharging losses
        labels_length = len(labels)
