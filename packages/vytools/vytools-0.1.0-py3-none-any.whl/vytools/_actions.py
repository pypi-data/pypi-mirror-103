...
def run(original_list, items=None, build_args=None, clean=False, save=False, data=None, data_mods=None, jobpath=None, cmd=None):
  if items is None: items=ITEMS
  if build_args is None: build_args = {}
  results = {}
  for type_name in original_list:
    if type_name.startswith('stage:'):
      build_arg_dict = build_args.copy()
      results[type_name] = vytools.stage.run(type_name, items=items, build_args=build_arg_dict, jobpath=jobpath, cmd=cmd)
    elif type_name.startswith('compose:'):
      build_arg_dict = build_args.copy()
      epid = type_name.replace(':','__')
      results[type_name] = vytools.compose.run(epid, type_name, items=items, build_args=build_arg_dict, jobpath=jobpath)
    elif type_name.startswith('episode:'):
      build_arg_dict = build_args.copy()
      results[type_name] = vytools.episode.run(type_name, items=items, build_args=build_arg_dict, save=save, clean=clean, data=data, data_mods=data_mods, jobpath=jobpath)
  return results
