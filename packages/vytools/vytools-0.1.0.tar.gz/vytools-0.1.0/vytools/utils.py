...
def ui_search(rootpath, vydir, func, items):
  for root, dirs, files in os.walk(rootpath, topdown=True):
    for f in files:
      if not f.endswith('vydirectory'):
        pth = os.path.join(root,f)
        name = os.path.join(vydir, str(PurePath(pth).relative_to(rootpath)))
        func(name, pth, '', items)
...
def get_repository_paths(rep_str_lst, contextpaths=None):
  ...
  for item in repo_versions.values(): vytools.printer.print_def('  '+item)
    ...
    for cp in contextpaths:
      for root, dirs, files in os.walk(cp, topdown=True):
        ...
        if is_ui and func:
          ...
          if vydir: ui_search(root, vydir, func, items)
