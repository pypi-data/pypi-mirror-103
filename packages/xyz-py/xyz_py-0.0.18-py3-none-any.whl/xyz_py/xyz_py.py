...
def load_xyz(f_name: str, atomic_numbers: bool = False):
    ...
    Keyword arguments:
        atomic_numbers (bool) : If true, will read xyz file with atomic numbers
                                and convert to labels

    Returns:
        _labels (list) : atomic labels
        _coords (list) : list of 3 element lists containing xyz coordinates
                         of each atom
    """
...
def save_xyz(f_name: str, labels: list, coords: list,
    ...
    Positional arguments:
        ...
        coords (list) : list of 3 element lists containing xyz coordinates of
                        each atom

    Keyword arguments:
        with_numbers (bool)   : If True, add/overwrite numbers to labels before
                                printing
        verbose (bool)        : Print information on filename to screen
        mask (list)           : n_atom list of 0 (exclude) and 1 (include)
                                indicating which atoms to print
...
def get_neighborlist(labels: list, coords: list, adjust_cutoff: dict = {}):
    ...
    Keyword arguments:
        adjust_cutoff (dict) : dictionary of atoms (keys)
                               and new cutoffs (values)
...
def get_adjacency(labels: list, coords: list, adjust_cutoff: dict = {},
    ...
    Returns:
        adjacency (numpy array) : Adjacency matrix with same order
                                  as labels/coords
    """
...
def remove_broken(labels: list, coords: list, formulae: list,
    ...
    Returns
        ...
        - dictionary containing indices of complete molecules, where
          the indices correspond to the original ordering of labels and
          coords given to this function
    ...
    Keyword arguments:
        adjust_cutoff (dict)  : dictionary of atoms (keys) and
                                new cutoffs (values)
    ...
    Returns:
        ...
        coords_clean (list)  : list of lists of xyz coordinates of each atom
                               for full molecules
        labels_fragments (list) : list of atomic labels for fragments
        coords_fragments (list) : list of lists of xyz coordinates of each
                                  atom for fragments
        mol_indices     (dict) : dictionary containing indices of complete
                                 molecules -
...
def _mask_list(_list: list, mask: list, opp_mask: bool = False):
    ...
    Keyword arguments:
        opp_mask (bool) : Apply mask in opposite fashion:
...
def _calculate_rmsd(coords_1: list, coords_2: list):
    ...
    Returns:
        rmsd (float) : Root mean square of norms of deviation
...
def calculate_rmsd(coords_1: list, coords_2: list, mask_1: list = [],
    ...
    Keyword arguments:
        mask_1 (list)  : list of 0 (exclude) and 1 (include) for each
                            element in coords_1
        mask_2 (list)  : list of 0 (exclude) and 1 (include) for each
                            element in coords_2
        order_1 (list) : list of new indices for coords_1
                            - applied BEFORE masking
        order_2 (list) : list of new indices for coords_2
                            - applied BEFORE masking

    Returns:
        rmsd (float) : Root mean square of norms of deviation between
                        two structures
    """
...
def rotate_coords(coords: list, alpha: float, beta: float, gamma: float):
    ...
    Returns:
        rot_coords (list) : list of lists of xyz coordinates of each
                            atom after rotation
    """
...
def _rotate_and_rmsd(angs: list, coords_1: list, coords_2: list):
    ...
    Returns:
        rot_coords (list) : list of lists of xyz coordinates of each atom
                                after rotation
    """
