...
class LabelModel(Model):
    ...
    def add_compound(self, compound: str, is_isotopomer=False, **meta_info):
    ...
    def _add_base_compound(self, *, base_compound: str, num_labels: int, label_names: list[str], **meta_info):
        """Add a base compound of label isotopomer."""
        self.label_compounds[base_compound] = {
            "num_labels": num_labels,
            "isotopomers": label_names,
        }
        self._check_and_insert_ids([base_compound])
        self.meta_info.setdefault("compounds", {}).setdefault(base_compound, Compound(**meta_info))

    def _add_isotopomers(self, *, base_compound: str, label_names: list[str]):
    ...
    def add_label_compound(self, compound: str, num_labels: int, **meta_info):
    ...
    def add_label_compounds(self, compounds: dict[str, int], meta_info=None):
    ...
    def remove_compound(self, compound: str, is_isotopomer=False):
    ...
    def remove_label_compound(self, compound: str):
    ...
    def remove_label_compounds(self, compounds: list[str]):
    ...
    @staticmethod
    def _split_label_string(label: str, *, labels_per_compound: list[str]) -> list[str]:
    ...
    @staticmethod
    def _map_substrates_to_products(*, rate_suffix: str, labelmap: list[int]) -> list[str]:
        """Map the rate_suffix to products using the labelmap."""
        return "".join([rate_suffix[i] for i in labelmap])

    @staticmethod
    def _unpack_stoichiometries(*, stoichiometries: dict[str, int]) -> tuple[list[str]]:
    ...
    @staticmethod
    def _repack_stoichiometries(*, new_substrates: list[str], new_products: list[str]) -> dict[str, int]:
    ...
    def _create_isotopomer_reactions(
        ...
        *,
        ...
        base_substrates: list[str],
        base_products: list[str],
        labels_per_substrate: list[str],
        labels_per_product: list[str],
    ):
        variants = []
    ...
    def add_labelmap_reaction(
        ...
        stoichiometry: dict[str, float],
        labelmap: list[int],
        external_labels: Union[int, list[int]] = None,
        modifiers: list[str] = None,
        parameters: list[str] = None,
    ...
    def remove_labelmap_reaction(self, rate_name: str):
    ...
    def remove_labelmap_reactions(self, rate_names: list[str]):
    ...
    def generate_y0(
        self, base_y0: dict[str, float], label_positions: dict[str, Union[int, list[int]]] = None
    ) -> dict[str, float]:
    ...
    def _create_label_scope_seed(self, *, initial_labels: dict[str, int]) -> dict[str, int]:
    ...
    def _create_sbml_compounds(self, *, sbml_model: libsbml.Model):
