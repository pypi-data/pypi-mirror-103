...
class BaseModel(ABC):
    ...
    def _check_for_existence(self, *, name: str, check_against: list[str], candidates: list[str]):
        check_against = set(check_against)
        check_against.add("time")
        for i in set(candidates).difference(check_against):
            warnings.warn(f"Could not find {i} for {name}")

    def _check_and_insert_ids(self, ids: list[str]):
        for id_ in ids:
            if id_ in self._ids:
                warnings.warn(f"Id {id_} already exists in the model. This might lead to variable shading!")
            self._ids.add(id_)

    def _update_ids(self, ids: dict[str, str]):
    ...
    def update_meta_info(self, component: str, meta_info: dict):
    ...
    def _get_nonzero_meta_info(self, *, component: str):
    ...
    def _create_sbml_units(self, *, sbml_model: libsbml.Model):
    ...
    def _create_sbml_compartments(self, *, sbml_model: libsbml.Model):
    ...
    def write_sbml_model(self, filename: str = None):
