...
class _BaseSimulator(ABC):
    def __init__(self, model, integrator_name: str, **kwargs):
        ...
        self.y0: list[float] = kwargs.get("y0")
        self.time: list[float] = kwargs.get("time")
        self.results: list[list[float]] = kwargs.get("results")

        # Placeholders
        self.integrator = None
        self.default_integrator_kwargs = None

    def __reduce__(self) -> _BaseSimulator:
        """Pickle this class."""
        return (
            self.__class__,
            (
    ...
    def _initialise_integrator(self, *, y0: dict[str, float]):
    ...
    def initialise(self, y0: Union[float, list[float]], test_run=True):
        """Initialise the integrator."""
    ...
    def _save_simulation_results(self, *, time: list[float], results: list[float], skipfirst: bool):
    ...
    def get_time(self, concatenated: bool = True) -> Union[np.ndarray, list[np.ndarray]]:
        """Get simulation time.
    ...
    def store_results_to_file(self, filename: str, filetype: str = "json"):
    ...
    def load_results_from_file(self, filename: str, filetype: str = "json"):
