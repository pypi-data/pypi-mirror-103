...
from stonky.utils import (buy_crypto, create_data, get_balance, load_data,
                          save_data, sell_crypto)
...
class MeanTrend:
    ...
    def iteration(self, main_crypto_data):
        for pair in self.pairs:
            ...
            if trades.get(pair) and trades.get(pair)[-1].get("action") == "buy":
                ...
                if action:
                    sell_crypto(self.name, self.currency, *action)
            else:
                action = self._action(pair, crypto_data, crypto_stored)
                if action:
                    buy_crypto(self.name, self.currency, *action)
    ...
    def _action(self, name, crypto_data, crypto_stored, buy=True):
        ...
        if make_trade:
            ...
            if buy:
                money = balance.get(self.currency)
