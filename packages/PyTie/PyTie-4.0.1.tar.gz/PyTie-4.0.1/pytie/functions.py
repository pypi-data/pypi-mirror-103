...
def int_data(las_files, N_files, DT_id, RHOB_id, Depth_id):
    ...
    DT = DT log integrated - Pandass DataFrame
    RHOB = RHOB log integrated - Pandas DataFrame
...
def fill_gaps(las_files, N_files, DT, RHOB):
    ...
    las_file = add the folder and file(s) address(es). 
    N-files = attribute the number of files to be imported.
    DT = DT log integrated - Pandas DataFrame
    RHOB  = RHOB log integrated - Pandas DataFrame
...
def time_depth(N_files, las_files, DT, DEPTH):
    ...
    time_depth_table = Pandass DataFrame
...
def export_synt(DEPTH, seism_freq, N_files, las_files):
    ...
    DEPTH_r = DEPTH[:-1]
    DEPTH_a = np.array([DEPTH_r])
    seism_freq_a = np.array([seism_freq])
...
def resampling_data(resample_interval, data, id_data, id_depth):
    """
    Ex.: DEPTH, DATA_resampled = pytie.resampling_data(sample_interval, data)
    ...
    """
    file_data = np.loadtxt(data, delimiter=',' )
    
    START = file_data[0,1]
    END = file_data[-1,1]
        
    a = np.round(np.arange(END- sample_interval, START, -sample_interval), decimals=2)
    
    DEPTH = np.sort(a, axis=None)
    
    DEPTH_data = file_data[:,1]
    DATA = file_data[:,0]
...
def transform_aratu_sad69(las_files, N_files):
    ...
    if N_files == 1:
        ...
        for i in range(N_files):
            X = float(log0.well.fl1.value[5:15])
        ...
        Y_Aratu = []
        for i in range(N_files):
            Y = float(log0.well.fl1.value[25:34])
        ...
        well = str(log0.well.well.value)
        well = well.replace(" ", "")
        well_name = np.array([np.append(well_name, well)])
    
        K = (log0.well.EKB.value)
        KB = np.array([np.append(KB, K)])
    
        total = (log0.well.STOP.value)
        total_depth = np.array([np.append(total_depth, total)])
    
        well_a = str(log0.well.api.value)
        well_api = np.array([np.append(well_api, well_a)])
    
        M = total + K
        MD = np.array([np.append(MD, M)])
    
        SAD69 = np.hstack((well_name.T, X_2.T, Y_2.T, KB.T, total_depth.T, MD.T, well_api.T))
    
        np.savetxt(well+'_well_track.txt', SAD69, fmt='%s', delimiter='    ', newline='\n', header='well_name         X            Y             KB          TVDSS         MD                   well_api', footer='', comments='# ') 
...
def markers_tie(las_files, N_files, markers, depths_markers):
    ...
    time_depth_table = Pandass dataframe with time and depth
