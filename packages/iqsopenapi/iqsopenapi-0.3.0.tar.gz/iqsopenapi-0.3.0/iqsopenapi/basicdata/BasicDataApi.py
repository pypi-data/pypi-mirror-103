...
if __name__ == '__main__':
    ...
    begin = datetime.datetime(2021, 4, 23, 20, 30, 0)
    end = begin + datetime.timedelta(days=1)
    open_times = api.GetOpenTimes(begin,end)

    if open_times is None or len(open_times) <= 0:
        pass
    night_time = begin + datetime.timedelta(hours=1)
    day_time = begin + datetime.timedelta(hours=13)
    for x in open_times:
        if (night_time >= datetime.datetime.strptime(x.begin,'%Y%m%d%H%M%S') and night_time <= datetime.datetime.strptime(x.end,'%Y%m%d%H%M%S')) or (day_time >= datetime.datetime.strptime(x.begin,'%Y%m%d%H%M%S') and day_time <= datetime.datetime.strptime(x.end,'%Y%m%d%H%M%S')) :
