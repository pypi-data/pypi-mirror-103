...
def compute_masks2(event):
    print(cellprob_threshold, model_match_threshold)

def _on_init(widget):
    widget.compute_masks_button.changed.connect(compute_masks2)
...
@magic_factory(widget_init=_on_init, 
                ...
                compute_masks_button  = dict(widget_type='PushButton', text='recompute last masks with new cellprob + model match', enabled=True),
