...
class PortfolioDataApi(metaclass=Singleton):
    ...
    def get_all_fund_nav(self, start_date=None, end_date=None, fund_list: Tuple[str] = ()):
        try:
            ...
            if len(fund_list) > 0:
                ...
                hedge_fund_list = [_ for _ in fund_list if '!' not in _]
                if len(mutual_fund_list) > 0:
                    mutual_fund_nav = BasicDataApi().get_fund_nav_with_date(start_date=start_date,end_date=end_date,fund_list=mutual_fund_list)
                if len(hedge_fund_list) > 0:
                    hedge_fund_nav = RawDataApi().get_hf_fund_nav(fund_ids=hedge_fund_list, start_date=start_date, end_date=end_date)
            else:
                mutual_fund_nav = BasicDataApi().get_fund_nav_with_date(start_date=start_date,end_date=end_date)
                hedge_fund_nav = RawDataApi().get_hf_fund_nav_dt(start_date=start_date, end_date=end_date)
    ...
    def get_port_recent_ret_details(self, trade_history, index_id, time_para, begin_date, end_date, benchmark_id_info):
        try:
            ...
            exsited_index_list = fund_type_wgt[fund_type_wgt>0].dropna().index.tolist()
            ...
            # if remove not existed asset
            #result_2 = result_2[exsited_index_list]
    ...
    def calc_port_status(self, trade_history):
        try:
            ...
            dic = {
                'last_nav':p_s['last_unit_nav'],
                'last_increase_rate':p_s['last_increase_rate'],
                'end_date':str(p_s['end_date']),
                'annual_ret':p_s['annualized_ret'],
                'annual_vol':p_s['annualized_vol'],
                'mdd':p_s['mdd'],
                'sharpe':p_s['sharpe'],
                'calmar':p_s['ret_over_mdd'],
                'recent_y1_ret':p_s['recent_y1_ret'],
                'recent_y3_ret':p_s['recent_y3_ret'],
                'recent_y5_ret':p_s['recent_y5_ret'],
            }
            return dic
