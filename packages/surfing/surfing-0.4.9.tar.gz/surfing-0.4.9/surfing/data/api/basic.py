...
class BasicDataApi(metaclass=Singleton):
    ...
    def get_asset_benchmark_info_with_layers(self):
        try:
            ...
            asset_info_1 = asset_info[asset_info.asset_type != '策略指数'].copy()
            asset_info_2 = asset_info[asset_info.asset_type == '策略指数'].copy()
            real_id_1 = BasicDataApi().get_index_info(index_list=asset_info_1.real_id.tolist()).set_index('index_id')['em_id'].to_dict()
            asset_info_1.loc[:,'others_id'] = asset_info_1.real_id.map(real_id_1)
            asset_info_1 = asset_info_1.dropna()
            asset_info_2.loc[:,'others_id'] = '融智指数'
            asset_info = pd.concat([asset_info_1,asset_info_2])
            ...
            for i in asset_types:
                ...
                for r in _df_i.itertuples():
                    _dic = {'asset_id':r.asset_id, 'desc_name':r.real_name, 'others_id':r.others_id}
