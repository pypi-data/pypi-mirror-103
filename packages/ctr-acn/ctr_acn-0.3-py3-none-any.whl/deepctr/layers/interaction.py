...
class FECNNLayer(Layer):
    ...
    def __init__(self, filters=(14, 16,), kernel_width=((7, 1), (7, 1)), pooling_width=((2, 1), (2, 1)),
                 **kwargs):
    ...
    def build(self, input_shape):
        ...
        for i in range(1, len(self.filters) + 1): 
            ...
            pooling_shape = self._pooling_output_shape(conv_output_shape, pooling_width)
            #self.conv_layers.append(tf.keras.layers.Conv2D(filters=filters, kernel_size=(width, 1), strides=(1, 1),
            self.conv_layers.append(tf.keras.layers.Conv2D(filters=filters, kernel_size=width, strides=(1, 1),
                                                           padding='same',
                                                           activation='relu', use_bias=True, ))
            self.pooling_layers.append(tf.keras.layers.MaxPooling2D(pool_size=pooling_width))
