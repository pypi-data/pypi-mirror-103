...
class AFM:
    ...
    def nanosurf_import(
        ...
        self.approach_raw = _approach
        self.retract_raw = _withdraw
        self.z_approach = _z_approach
        self.z_withdraw = _z_withdraw
    ...
    def plot_raw(
        ...
        if self.file_name[-3:] != 'nid':
            ...
            fig = go.Figure()
            fig.add_trace(go.Scatter(y = self.approach_raw[:,curve_number], x = self.z_approach[:,curve_number], mode='lines', name = 'Approach curve (raw)'))
            fig.add_trace(go.Scatter(y = self.retract_raw[:,curve_number], x = self.z_withdraw[:,curve_number], mode='lines', name = 'Retract curve (raw)'))
            fig.update_layout(
                yaxis_title_text='Force (nN)',
                xaxis_title_text='Piezo position (nm)',
            )
    ...
    def baseline(
        ...
        self.z_approach = np.zeros([self.samps_line, len(self.filelist)])
        self.z_withdraw = np.zeros([self.samps_line, len(self.filelist)])
        ...
        for curve_number in range(0, self.z_position_raw.shape[1]):
            ...
            if approach_dev < max_approach_noise and retract_dev < max_retract_noise:
                ...
                if self.file_name[-3:] != 'nid':
                    self.z_approach[:,i] = piezo_position
                    self.z_withdraw[:,i] = piezo_position
