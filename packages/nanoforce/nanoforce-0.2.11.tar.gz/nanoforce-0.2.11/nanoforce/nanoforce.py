...
class AFM:
    ...
    def baseline(
        ...
        for curve_number in range(0, self.measurements):
            ...
            if approach_dev < max_approach_noise and retract_dev < max_retract_noise:
                ...
                if self.file_name[-3:] != 'nid':
                    ...
                    self.z_approach[:,i] = _z_a[::-1] * -1 * 1e-9
                    self.z_withdraw[:,i] = _z_w[::-1] * -1 * 1e-9
                    self.approach[:,i] = adjusted_approach[::-1]
                    self.retract[:,i] = adjusted_retract[::-1]
        ...
        if self.file_name[-3:] != 'nid':
            self.z_approach = self.z_approach[:,:i] * 1e-9
            self.z_withdraw = self.z_withdraw[:,:i] * 1e-9
