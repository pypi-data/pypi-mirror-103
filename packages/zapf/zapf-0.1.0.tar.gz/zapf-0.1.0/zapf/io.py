...
class Cache:
    ...
    def update_range(self, addr, length):
        with self.lock:
            ...
            if self.range[0] == self.range[1]:
                ...
                first = min(self.range[0], addr)
                above = max(self.range[1], addr + length)
            # cache range stores (start, end+1)
            self.range = first, above

    def get(self, addr, length):
        with self.lock:
            if addr >= self.cached_range[0] and \
               addr + length <= self.cached_range[1]:
                addr -= self.cached_range[0]
    ...
    def sync(self):
        ...
        with self.lock:
            if self.range != (0, 0):
                try:
                    cached_data = self.proto.read(self.range[0],
