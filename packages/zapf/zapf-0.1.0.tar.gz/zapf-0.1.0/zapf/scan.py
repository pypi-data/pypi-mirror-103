...
class Scanner:
    ...
    def get_devices(self, _device_filter=None):
        ...
        for name, addr, typecode, info in self.scan_devices():
            devcls = Device.class_for(typecode)
            if devcls is None:
                self.log.warning('type code %#x is not supported, skipping',
                                 typecode)
            else:
                yield devcls(name, addr, typecode,
                             info['params'], info['funcs'], self.io, self.log)
    ...
    def scan_2015_02(self):
        ...
        next_addr = indexer.addr + indexer.size

        for devnum in range(1, 256):
            # TODO: again this expects the minimum indexer size of 22
            ...
            if devclass in (1, 2):
                ...
                for p in param_ids:
                    if 128 <= p <= 191 or 224 <= p <= 255:
                        functions.append(spec.SpecialFunctions[p])
                    else:
