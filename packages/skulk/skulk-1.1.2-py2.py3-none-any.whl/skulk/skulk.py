...
def _get_version(version_file):
    ...
    try:
        ...
        sys.stderr.write(
            "Can't get version from the version file. {}\n".format(version_file))
...
def _resolve_version(repo, version, version_file):
    ...
    do_bump = True
    if not valid:
        print("Version invalid: ({}) must be greater than ({})".format(
            version, versions[-1]))
    else:
        print(
            "Version is valid ({}). However, you may bump it if you wish.".format(version))
...
def _check_pre_push_hook(repo):
    ...
    if os.path.exists(hook_file):
        ...
        do_continue = _yes_no("Do you want to continue without the hook, or exit skulk while you add the hook code? Continue?")
        if do_continue:
            return
        sys.exit(1)
    ...
    if do_create:
        ...
        print(
            "No worries. Just be sure to always use the skulk command, and not git push\n")


def _show_auto_hook_preamble():
    sys.stderr.write(
        "There's no pre-push git hook active in this repository.\n")
    sys.stderr.write(
...
def _show_manual_hook_instructions(hook_file, HOOK_CONTENT):
    sys.stderr.write(
        "A pre-push git hook exists, but it doesn't contain the skulk check.\n")
    sys.stderr.write(
        "It is recommeded to create the hook so you don't push without a \n")
    sys.stderr.write("version bump or an up to date changelog.\n")
