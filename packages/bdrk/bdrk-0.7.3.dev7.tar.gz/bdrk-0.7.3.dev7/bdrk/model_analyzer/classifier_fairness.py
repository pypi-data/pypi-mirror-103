...
class ClassifierFairness(Fairness):
    ...
    def _get_fairness(
        ...
        config_attributes = set(available_attributes + unprivileged_attribute_values)
        invalid_attributes = config_attributes.difference(available_attributes)
        if (invalid_attributes):
            logger.error(f"Attribute {invalid_attributes} does not exist in feature {protected_attribute}")
