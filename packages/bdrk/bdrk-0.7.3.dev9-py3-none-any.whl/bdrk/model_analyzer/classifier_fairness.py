...
class ClassifierFairness(Fairness):
        ...
        for attr, attr_vals in self.fconfig.items():
            if attr not in self.features:
                ...
                for cl in labels_cls:
                    ...
                    if fai_data is not None:
                        fairness_metrics[attr][f"class {cl}"] = fai_data
    ...
    def _get_fairness(
        ...
        if (invalid_attributes):
            logger.error(f"Attributes {invalid_attributes} do not exist in feature {protected_attribute}")
            return None
