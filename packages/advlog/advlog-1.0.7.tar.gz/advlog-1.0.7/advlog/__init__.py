...
class Logger:
    ...
    def __init__(self, logfile: Optional[str] = None, *, timestamp_format: Optional[str] = "%y/%m/%d-%H:%M:%S.%f", encoding: str = 'utf8'):
    ...
    def log(self, level: LogLevel, *args, file: Optional[typing.IO] = None):
        ...
        if self.filepath:
            with open(self.filepath, 'a') as fd:
                fd.write(msg.encode(self.encoding))
    
    def _buildmsg(self, level: LogLevel, args: Sequence):
        sio = StringIO()
