...
def build(serviceName, version, base_url, **kw):
    ...
    Usage Examples - Data Services:
        ...
        api_response = service.accounts().contacts().list(account=account)
        api_response = service.accounts().contacts().list(account_uid=account_uid)
        api_response = service.accounts().contacts().insert(new_account_contact=new_account_contact)
        api_response = service.accounts().contacts().update(account_contact=account_contact)
        api_response = service.accounts().contacts().delete(account_contact_uid="123e4567-e89b-12d3-a456-426614174000")
...
class DataServicesResource(object):
    ...
    class __Accounts(Consumer):
        ...
        class __Customers(Consumer):
            ...
            def dealer_information(self):
                return self.__DealerInformation(self)
            ...
            @headers({"Ocp-Apim-Subscription-Key": key})
            class __DealerInformation(Consumer):
                """Inteface to dealer information resource for the RockyRoad API."""

                def __init__(self, Resource, *args, **kw):
                    super().__init__(base_url=Resource._base_url, *args, **kw)

                @returns.json
                @get("accounts/customers/dealer-information")
                def list(
                    self,
                    ...
                    """This call will return dealer information for the account specified."""

                @returns.json
                @json
                @patch("accounts/customers/dealer-information")
                def update(self, dealer_information: Body):
                    """This call will update dealer information with the specified parameters."""
