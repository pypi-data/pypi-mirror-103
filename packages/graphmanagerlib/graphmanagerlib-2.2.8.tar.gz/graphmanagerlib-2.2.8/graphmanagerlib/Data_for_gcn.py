...
from graphmanagerlib.JsonManager import ConvertJsonToDocumentsObjects, GetJsonFromFile
...
def getDataForUniquePrediction(jsonPath,saved_prediction_dataset_folder):
    test_list_of_graphs = []
    jsonAsObject = GetJsonFromFile(jsonPath)
    jsonAsDoc = ConvertJsonToDocumentsObjects(jsonAsObject)[0]
    individual_data = makeIndividualData(jsonAsDoc)
    test_list_of_graphs.append(individual_data)
    test_data = torch_geometric.data.Batch.from_data_list(test_list_of_graphs)
    test_data.edge_attr = None
    saved_path = os.path.join(saved_prediction_dataset_folder, 'predictions_data.dataset')
    torch.save(test_data, saved_path)
