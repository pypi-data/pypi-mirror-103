...
from mitoinstaller.log_utils import analytics, log, log_failed_install
...
def install_step_create_user_install():
    ...
    # If the user has not static install ID, create one
    if static_user_id_install is None:
        create_user_install()    
        # Reread the ID, as it is now defined
        static_user_id_install = get_static_user_id_install()
    
    analytics.identify(static_user_id_install)
...
def install_step_install_mitosheet():
    # Then, we install all the packages required for Mito
...
def install_step_finish_install():
    ...
    set_user_field('finished_install', True)
    log('finished_install')
...
def install():
    for install_step in INSTALL_STEPS:
        try:
            ...
            log_failed_install(error_message)
