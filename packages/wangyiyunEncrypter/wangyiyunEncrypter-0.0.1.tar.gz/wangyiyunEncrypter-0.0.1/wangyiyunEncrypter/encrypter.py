...
# 网易云加密函数
class WangyiyunEncrypter(object):
    def __init__(self):
        js = WangyiyunEncrypterJS().js
        self.func = execjs.compile(js)

    # 构造用于获取歌曲真实url的data
    # song_id --> 歌曲id
    # 返回 --> 构造好的data字典
    def construct_song_data(self, song_id):
        info = {
            "ids": "[%s]" % song_id,
            "level": "standard",
            "encodeType": "aac",
        ...
        data = self.func.call('construct', str(info))
        return data

    # 构造用于获取歌词的data
    def construct_lyric_data(self, song_id):
        info = {
            "csrf_token": "",
            "id": "%s" % song_id,
            "lv": -1,
    ...
    # 构造用于获取评论的data
    def construct_comments_data(self, pageNo, pageSize, song_id):
        info = {
            "csrf_token": "",
            "cursor": "-1",
            "offset": "0",
    ...
    # keywords --> 歌曲名等关键词
    # limit --> 搜索返回的推荐数
    def construct_search_data(self, keywords, limit):
        info = {
            "csrf_token": "",
            "hlposttag": "</span>",
...
class WangyiyunEncrypterJS(object):
    def __init__(self):
        self.js = '''
// AES加密所用到的所有函数
(function() {})();
...
var asrsea = d,
    bqD8va = "010001",
    bqD8vb = "",
    bqD8vc = "0CoJUm6Qyw8W8jud";

// 加密主函数, 调用此函数生成data
function construct(info) {
        var bVX6R = asrsea(info, bqD8va, bqD8vb, bqD8vc);
        data = {
            "params": bVX6R.encText,
            "encSecKey": bVX6R.encSecKey
        };
        return data;
    }
'''
