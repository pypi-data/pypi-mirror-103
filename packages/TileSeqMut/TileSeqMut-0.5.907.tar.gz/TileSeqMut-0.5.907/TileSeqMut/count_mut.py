...
class readSam(object):
    ...
    def adjust_er(self):
        ...
        if self._sample_condition in wt["Condition 1"].tolist() and self._sample_rep == 1:
            ...
            wt_name = wt[wt["Condition 2"] == self._sample_condition]["Condition 1"]
            # get wt sample for this tile and rep 1
            wt_id = self._samples[(self._samples["Condition"] == wt_name) & (self._samples["Tile ID"] == self._sample_tile) & (self._samples["Replicate"] == 1)].values[0]
            # check if calibrated
        ...
        if not os.path.isfile(phred_output_r1):
            ...
            cmd_r1 = f"calibratePhred.R {self._r1} -p {self._param} -o {phred_output_r1} -l {log_f}"
            os.system(cmd_r1)
        if not os.path.isfile(phred_output_r2):
            ...
            cmd_r2 = f"calibratePhred.R {self._r2} -p {self._param} -o {phred_output_r2} -l {log_f}"
            os.system(cmd_r2)
