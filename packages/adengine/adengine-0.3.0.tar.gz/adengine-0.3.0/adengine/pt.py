...
def _get_packet_tracer_process(port: int) -> psutil.Process or None:
    ...
    Returns:
        psutil.Process instance of PacketTracer process, if process found, otherwise None.
    """
    pt_processes = [process for process in psutil.process_iter() if process.name() == PT_EXECUTABLE_NAME]
    for process in pt_processes:
        if '--ipc-port' in process.cmdline():
            ipc_port_arg_index = process.cmdline().index('--ipc-port')
            process_port = int(process.cmdline()[ipc_port_arg_index + 1])
            if port == process_port:
                return process
    return None


def _launch_pt(port=39000, nogui=False, timeout: int = -1, use_virtual_display=False) -> psutil.Process:
    ...
    Returns:
        psutil.Process instance of launched PacketTracer process.
    Raises:
    ...
    if timeout == -1:
        ...
        if not pt_is_up():
            _get_packet_tracer_process(port).terminate()
    ...
    return _get_packet_tracer_process(port)
...
class PacketTracerSession:
    ...
    If port not specified in constructor, random free port will be used instead.
    ...
    def __init__(self, port: int = 0, nogui=False, use_virtual_display=False):
        self._port = port or _get_free_port()
        self._nogui = nogui
    ...
    def start(self):
        if not self._process:
            ...
            self._process = _launch_pt(self._port, self._nogui, use_virtual_display=self._use_virtual_display)
            logger.info(f'Launched Packet Tracer session on port {self._port} with pid {self._process.pid}')
        else:
            raise PacketTracerSessionAlreadyRunning(self._port, self._nogui)

    def stop(self):
        if self._process:
            logger.debug(f'Stop Packet Tracer on port {self._port} with pid {self._process.pid}')
            if not self._process.is_running():
                logger.info(f'Packet Tracer process on port {self._port} with pid {self._process.pid} already stopped')
            else:
                self._process.terminate()
                logger.info(f'Stopped Packet Tracer session on port {self._port} with pid {self._process.pid}')
            self._process = None
        else:
