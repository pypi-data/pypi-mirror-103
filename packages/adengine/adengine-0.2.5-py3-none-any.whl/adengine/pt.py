...
class PacketTracerSession:
    ...
    def __init__(self, port: int = 0, nogui=False, use_virtual_display=False):
        if not port:
            self._generate_random_port = True
    ...
    def start(self):
        ...
        if not self._pid:
            ...
            logger.info(f'Launched Packet Tracer session on port {self._port} with pid {self._pid}')
        else:
    ...
    def stop(self):
        if self._pid:
            ...
            if not _process_alive(self._pid):
                logger.debug(f'Packet Tracer process on port {self._port} with pid {self._pid} already stopped')
            else:
    ...
    def running(self) -> bool:
        return self._pid and _process_alive(self._pid)
