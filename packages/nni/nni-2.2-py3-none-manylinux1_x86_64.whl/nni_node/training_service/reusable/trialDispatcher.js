...
let TrialDispatcher = class TrialDispatcher {
    constructor(config) {
        ...
        util_1.validateCodeDir(config.trialCodeDirectory);
        if (Array.isArray(config.trainingService)) {
            config.trainingService.forEach(trainingService => {
                const env = environmentServiceFactory_1.EnvironmentServiceFactory.createEnvironmentService(trainingService.platform, config);
                this.environmentServiceList.push(env);
            });
        }
        else {
            const env = environmentServiceFactory_1.EnvironmentServiceFactory.createEnvironmentService(config.trainingService.platform, config);
            this.environmentServiceList.push(env);
        }
