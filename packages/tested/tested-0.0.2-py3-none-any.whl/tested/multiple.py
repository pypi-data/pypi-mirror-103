...
def ensure_path(module):
    if isinstance(module, str) and os.path.exists(module):
        ...
        path = module.__file__  # TODO: Use other means, since __file__ doesn't work for some packages
        if path.endswith('__init__.py'):  # if path is an __init__.py
            path = os.path.dirname(path)  # ... take the containing folder
        return path


def multiple_pytests_commands(*modules_to_test, options: str = '--doctest-modules --quiet --disable-warnings'):
    template = "pytest " + options + " {path}; "
    for path in map(ensure_path, modules_to_test):
        yield template.format(path=path)


def multiple_pytests_command_str(*modules_to_test, options: str = '--doctest-modules --quiet --disable-warnings'):
    return "".join(multiple_pytests_commands(*modules_to_test, options=options))


def run_multiple_pytests(*modules_to_test, options: str = '--doctest-modules --quiet --disable-warnings',
                         dry_run=False):
