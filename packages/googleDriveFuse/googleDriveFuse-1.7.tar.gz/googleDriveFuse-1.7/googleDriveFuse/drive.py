...
import googleDriveFuse.constants
import googleDriveFuse.config
import googleDriveFuse.conf
import googleDriveFuse.chunked_download
import googleDriveFuse.errors
import googleDriveFuse.oauth_authorize
import googleDriveFuse.normal_entry
import googleDriveFuse.time_support
import googleDriveFuse.fsutility
            ...
            try:
                ...
                if error.get('code') == 403 and \
                   error.get('errors')[0].get('reason') \
                    ...
                    raise
            except googleDriveFuse.errors.AuthorizationFaultError:
                ...
                authorize = googleDriveFuse.oauth_authorize.get_auth()
...
class GdriveAuth(object):
    def __init__(self):
        self.__client = None
        self.__authorize = googleDriveFuse.oauth_authorize.get_auth()
    ...
    def get_client(self):
        if self.__client is None:
            ...
            discoveryUrl = \
                googleDriveFuse.conf.Conf.get('google_discovery_service_url')
...
class _GdriveManager(object):
    ...
    def list_changes(self, start_change_id=None, page_token=None):
        ...
        for item in items:
            ...
            if was_deleted:
                ...
                normalized_entry = \
                    googleDriveFuse.normal_entry.NormalEntry(
                        'list_changes',
                        entry)
    ...
    def get_children_under_parent_id(self,
        ...
        if query_is_string:
            query = ("title='%s'" %
                     (googleDriveFuse.fsutility.escape_filename_for_query(query_is_string)))
        elif query_contains_string:
            query = ("title contains '%s'" %
                     (googleDriveFuse.fsutility.escape_filename_for_query(query_contains_string)))
        else:
            query = None
    ...
    def get_entry(self, entry_id):
        ...
        return \
            googleDriveFuse.normal_entry.NormalEntry('direct_read', response)
    ...
    def list_files(self, query_contains_string=None, query_is_string=None,
        ...
        hidden_flags = googleDriveFuse.conf.Conf.get('hidden_flags_list_remote')
        if hidden_flags:
    ...
    def download_to_local(self, output_file_path, normalized_entry,
        ...
        if mime_type is None:
            ...
            elif googleDriveFuse.constants.OCTET_STREAM_MIMETYPE \
                    in normalized_entry.download_links:
                mime_type = googleDriveFuse.constants.OCTET_STREAM_MIMETYPE
        ...
        if mime_type != normalized_entry.mime_type and \
            ...
            raise googleDriveFuse.errors.ExportFormatError(message)
        ...
        with open(output_file_path, 'wb') as f:
            downloader = googleDriveFuse.chunked_download.ChunkedDownload(
                            f,
                            authed_http,
                            url)
    ...
    def create_directory(self, filename, parents, **kwargs):
        ...
        mimetype_directory = googleDriveFuse.conf.Conf.get('directory_mimetype')
    ...
    def __insert_entry(self, is_file, filename, parents, mime_type,
        ...
        now_phrase = googleDriveFuse.time_support.get_flat_normal_fs_time_from_dt()
        ...
        if googleDriveFuse.config.IS_DEBUG is True:
    ...
    def update_entry(self, normalized_entry, filename=None, data_filepath=None,
        ...
        if modified_datetime is not None:
            ...
            body['modifiedDate'] = \
                googleDriveFuse.time_support.get_flat_normal_fs_time_from_dt()
        ...
        normalized_entry = \
            googleDriveFuse.normal_entry.NormalEntry('update_entry', result)
    ...
    def rename(self, normalized_entry, new_filename):
        ...
        result = googleDriveFuse.fsutility.split_path_nolookups(new_filename)
