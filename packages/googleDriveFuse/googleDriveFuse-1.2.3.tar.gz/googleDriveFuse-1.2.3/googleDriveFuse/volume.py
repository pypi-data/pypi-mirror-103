...
import googleDriveFuse.utility
import googleDriveFuse.drive
import googleDriveFuse.account_info
import googleDriveFuse.normal_entry
import googleDriveFuse.cache_registry
import googleDriveFuse.cacheclient_base
...
#        logging.debug("Path [%s] does not exist for split.", path)
        raise googleDriveFuse.errors.GdNotFoundError()
...
class PathRelations(object):
    ...
    def get_instance():
        ...
        with PathRelations.rlock:
            try:
                return googleDriveFuse.cache_registry.CacheRegistry.__instance;
            except:
                pass

            googleDriveFuse.cache_registry.CacheRegistry.__instance = PathRelations()
            return googleDriveFuse.cache_registry.CacheRegistry.__instance
    ...
    def register_entry(self, normalized_entry):
        ...
        with PathRelations.rlock:
            ...
            if normalized_entry.__class__ is not googleDriveFuse.normal_entry.NormalEntry:
...
#                             "relations.", entry_id)
            ...
            for parent_id in parent_ids:
                ...
                while i <= 255:
                    ...
                    current_variation = \
                        filename_base + \
                        googleDriveFuse.utility.utility.translate_filename_charset(
                        ' (%d)' % (i,))
    ...
    def __load_all_children(self, parent_id):
        ...
        gd = googleDriveFuse.drive.get_gdrive()
...
#        it easier to keep track of a list of results.
                ...
                if num_results:
                    ...
                    parent_id = googleDriveFuse.account_info.AccountInfo.get_instance().root_id
...
#            self.__log.debug("Locating entry information for path [%s].", path)
            ...
            root_id = googleDriveFuse.account_info.AccountInfo.get_instance().root_id
...
class EntryCache(googleDriveFuse.cacheclient_base.CacheClientBase):
...
#        about = googleDriveFuse.account_info.AccountInfo.get_instance()
        self.__gd = googleDriveFuse.drive.get_gdrive()
    ...
    def __get_entries_to_update(self, requested_entry_id):
        ...
        considered_entries = {}
        max_readahead_entries = googleDriveFuse.conf.Conf.get('max_readahead_entries')
        for parent_id in parent_ids:
...
# individually.
    ...
    def get_max_cache_age_seconds(self):
        return googleDriveFuse.conf.Conf.get('cache_entries_max_age')
