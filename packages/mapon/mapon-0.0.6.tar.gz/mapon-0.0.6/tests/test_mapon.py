...
from mapon.config import _DBNAME, _GRAPH_NAME
from mapon.util import add, isnum, where
...
class TestLoading(unittest.TestCase):
    ...
    def test_loading_ordinary_csv(self):
        ...
        mo.run()
        self.assertEqual(len(list(mo.get('orders'))), mo._line_count('orders.csv', 'utf-8', '\n'))
    ...
    def test_loading_ordinary_tsv(self):
        ...
        mo.run()
        self.assertEqual(len(list(mo.get('markit'))), mo._line_count('markit.tsv', 'utf-8', '\n'))

    def test_loading_semicolon_separated_file(self):
        mo.register(orders1=mo.load('orders1.txt', delimiter=";"))
        mo.run()

        self.assertEqual(len(list(mo.get('orders1'))), mo._line_count('orders1.txt', 'utf-8', '\n'))
...
class TestApply(unittest.TestCase):
    ...
    def test_insert_empty_rows(self):
        ...
        mo.run()
        with self.assertRaises(mo.NoSuchTableFound):
            list(mo.get('orders1'))
...
class TestLogMsg(unittest.TestCase):
    def test_mute_log_messages(self):
        ...
        self.assertEqual(mo._CONFIG['msg'], True)
