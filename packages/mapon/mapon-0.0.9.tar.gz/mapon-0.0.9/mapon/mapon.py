...
from .config import (_CONFIG, _CONN, _DBNAME, _GRAPH_NAME, _JOBS,
                     _RESERVED_KEYWORDS, _TEMP)
...
def _execute(c, job):
    ...
    if cmd == 'load':
        ...
        if job['parallel'] and\
            ...
            _exec_parallel_apply(c, job, max_workers, tsize)
        else:
...
def _exec_parallel_apply(c, job, max_workers, tsize):
    itable = job['inputs'][0]
...
def register(**kwargs):
    ...
    .. code-block:: python
        ...
        mo.register(
            table_name = mo.load('sample.csv'),
            table_name1 = mo.apply(simple_process, 'table_name'),
        )
...
def run(**kwargs):
    ...
    :type ws: path as str
    :param max_workers: maximum workers for parallel work. The default is the
