...
def jam_hernquist_model_example():
    ...
    vrms = jam_axi_proj(surf, sigma, qObs, surf, sigma, qObs,
                        inc, mbh, distance, rad, rad*0).model
    ...
    vrms_maj = jam_axi_proj(surf, sigma, qObs, surf, sigma, qObs,
                            inc, mbh, distance, rad, rad*0, beta=beta).model
    vrms_min = jam_axi_proj(surf, sigma, qObs, surf, sigma, qObs,
                            inc, mbh, distance, rad*0, rad, beta=beta).model
    ...
    # Test the jam_axi_proj with Black Hole in the spherical isotropic limit
    #
    for bh in bhs:
        vrms = jam_axi_proj(surf, sigma, qObs, surf, sigma, qObs,
                            inc, bh, distance, rad, rad*0).model
