...
class PandasPraxis(widgets.DOMWidget):
    ...
    def handle_event(self, _, content, buffers):
        ...
        learn_easy = message[1]
        message = message[0]
        try:
            translation_list = translate_real(message, learn="easy" if learn_easy else "hard")
            translation_html = self.create_result(translation_list, alt_visibility = not learn_easy)
            # don't show alternatives if we are on easy mode
        except KeyError as e: 
            translation_html = str(e)
            
        self.send({'msg': translation_html})

    def create_result(self, translation_list, alt_visibility=True):
        ...
        for i, line in enumerate(translation_list):
            if 'other_options' in line:
                ...
                lines.append(self.create_or_line(i, line, alt_visibility))
            else:
                lines.append(self.create_not_or_line(i, line))
                
        return '<button id="pp-alt-toggle" onclick="switch_alts()">Toggle alternatives</button>' + \
            ''.join(explanations) + '<table id="pp-response-table"><tbody>' + ''.join(lines) + '</tbody></table>' + \
    ...
    def create_or_line(self, i, line, alt_visibility=True):
        item = line['first_choice']
        other_item = line['other_options'][0] # assuming only one other option
        return '''<tr>
                    <td class="unselectable"><i class="far fa-question-circle pp-or-button" style="visibility:{9}"
                ...
                <tr class="pp-below code" id="pp-below-{0}">
                    ...
                    <td class="selectable" id="pp-below-txt-{0}">{7}</td>
                        ...
                        item.column-1, item.end_column-1, other_item.value, item.value, 'visible' if alt_visibility else 'hidden')
