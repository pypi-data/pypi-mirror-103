...
class SalureFunctions:
    ...
    def cleanup_previous_store_actual(directory: str, actual_df: pd.DataFrame, remove_archived_files_after_days=31):
        ...
        os.makedirs(f'{directory}/archive_dir/', exist_ok=True)
        # Move files which are in the previous folder, to the archive folder.
        for file in os.listdir(f'{directory}/previous/'):
            os.rename(f'{directory}/previous/{file}', f'{directory}/archive_dir/{file}')
        ...
        for file in os.listdir(f'{directory}/archive_dir/'):
            if os.stat(f'{directory}/archive_dir/{file}').st_mtime < time.time() - remove_archived_files_after_days * 86400:
                os.remove(f'{directory}/archive_dir/{file}')

        # Store the new file in the actual folder if there is a new file given.
        actual_df.reset_index(drop=True, inplace=True)
        actual_df.to_feather(f'{directory}/actual/{int(time.time())}.ftr')
    ...
    def reverse_cleanup_files(directory: str):
        ...
        :param actual_dir: The full directory, including basedir, where the actual, previous and archive files are stored
        """
        for file in os.listdir(f'{directory}/actual/'):
