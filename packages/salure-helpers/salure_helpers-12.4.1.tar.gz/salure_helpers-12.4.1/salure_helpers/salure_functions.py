...
class SalureFunctions:
    ...
    @staticmethod
    def cleanup_files(actual_dir: str, previous_dir: str, archive_dir: str, remove_archived_files_after_days=31):
        """
        ...
        :param actual_dir: The full directory, including basedir, for the storage of new extracted files
        :param previous_dir: The full directory, including basedir, for the storage of files from the previous run (to compare with the current run)
        :param archive_dir: The full directory, including basedir, for the storage of older files for archiving reasons
        ...
        for file in os.listdir(previous_dir):
            os.rename(f'{previous_dir}{file}', f'{archive_dir}{file}')
        ...
        for file in os.listdir(actual_dir):
            os.rename(f'{actual_dir}{file}', f'{previous_dir}{file}')

        # Remove files in the archive folder if they're older than the given age in days
        for file in os.listdir(archive_dir):
            if os.stat(archive_dir + file).st_mtime < time.time() - remove_archived_files_after_days * 86400:
                os.remove(archive_dir + file)

    @staticmethod
    def reverse_cleanup_files(actual_dir: str, previous_dir: str, archive_dir: str):
        """
        ...
        """
        for file in os.listdir(actual_dir):
            os.rename(f'{actual_dir}{file}', f'{archive_dir}ERR-{file}')
        for file in os.listdir(previous_dir):
            os.rename(f'{previous_dir}{file}', f'{actual_dir}{file}')
