    ...
    def _tf_metrics_sinker(self, exec_properties):
        class S(MetricsSinker):
            def __init__(self):
                ...
                logging.info("[Metric Sinker]init")
                self._final_metric_value = dict()

            def __call__(self, ts, metrics, sample_ts, step):
                logging.debug("dump metrics step %s to %s" % (step, self._dir))
                logging.info("[Metric Sinker]call")
                logging.info("[Metric Sinker]metrics is %s", metrics)
                for name, value in metrics:
                    logging.info("[Metric Sinker]for name: %s, value: %s", name, value)
                    if isinstance(value, np.ndarray):
                        ...
                        for i, subvalue in enumerate(
                            ...
                            logging.info("[Metric Sinker]for summary: %s, subvalue: %s", summary, subvalue)
                    elif isinstance(value, (int, float)):
                        logging.info("[Metric Sinker]for summary: %s, value 1: %s", summary, value)
                        summary = tf.Summary(
                            value=[
                        ...
                        logging.info("[Metric Sinker]for summary: %s, value: %s", summary, value)
                    else:
                        raise RuntimeError(
            ...
            def close(self):
                logging.info("[Metric Sinker]close")
                fp = "{}/exp_{}/run_{}/{}".format(
                ...
                if self._final_metric_value:
                    logging.info("[Final Metric]exist")
                    ...
                    logging.info("[Final Metric]not exist")
                super(S, self).close()
