...
class XdlEvalRunner(XdlRunner):
    def run(self, reader: xdl.DataReader, eval_slots=[]):
        ...
        if not eval_slots:  # 正常eval
            logging.info("[XDL RUN] not eval_slots")
            with xdl.model_scope("phase0"):
                logging.info("[XDL RUN] batch is %s", batch)
                ret = self._model.process(batch=batch, phase=0,)
        else:  # eval slots
            logging.info("[XDL RUN] eval_slots")
            ret = ProcessResult()
        ...
        for name, tensor in ret.tensor_map.items():
            logging.info("[XDL RUN] type(tensor): %s", type(tensor))
            logging.info("[XDL RUN] tensor: %s", tensor)
        ...
        logging.info("[XDL RUN] train op: %s", train_ops)
        # ts tensor
