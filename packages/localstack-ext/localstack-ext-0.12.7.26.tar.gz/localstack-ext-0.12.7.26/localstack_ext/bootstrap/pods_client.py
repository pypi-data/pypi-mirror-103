...
class CloudPodManager(BhfcS):
 BACKEND='_none_'
 def __init__(self,pod_name=BhfcD,config=BhfcD):
  self.pod_name=pod_name
  self._pod_config=config
 def push(self):
  raise BhfcN('Not implemented')
 def pull(self):
  raise BhfcN('Not implemented')
 ...
 def restart_container(self):
  ...
  def check_status():
   ...
   assert Bhfct(statuses)==Bhfct(['running'])
 ...
 def get(cls,pod_name):
  ...
  raise BhfcN('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @Bhfcm
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise BhfcN('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==Bhfct([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=BhfcM)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise BhfcN('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
  ...
  for folder in PERISTED_FOLDERS:
   ...
   cp_r(src_folder,tgt_folder,rm_dest_on_conflict=BhfcM)
  self.restart_container()
 def client(self):
  client,path=get_transport_and_path_from_url(self.pod_config.get('url'))
  return client,path
 def local_repo(self):
  self.clone_dir=Bhfcz(self,'clone_dir',BhfcD)
...
class PodConfigManagerMeta(Bhfcg):
 def __getattr__(cls,attr):
  def _call(*args,**kwargs):
   result=BhfcD
   for manager in cls.CHAIN:
    try:
     tmp=Bhfcz(manager,attr)(*args,**kwargs)
     if tmp:
      if not result:
       result=tmp
      elif Bhfco(tmp,BhfcU)and Bhfco(result,BhfcU):
       result.extend(tmp)
    except BhfcN:
     pass
   if result is not BhfcD:
    return result
   raise BhfcN('Unable to run operation "%s" for local or remote configuration'%attr)
  return _call
class PodConfigManager(BhfcS,metaclass=PodConfigManagerMeta):
 CHAIN=[]
 @Bhfcm
 def pod_config(cls,pod_name):
  ...
  if not pod_config:
   raise BhfcN('Unable to find config for pod named "%s"'%pod_name)
  return pod_config[0]
class PodConfigManagerLocal(BhfcS):
 CONFIG_FILE='.localstack.yml'
 def list_pods(self):
  local_pods=self._load_config(safe=BhfcM).get('pods',{})
  local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
  existing_names=Bhfct([pod['pod_name']for pod in local_pods])
  result=[pod for pod in local_pods if pod['pod_name']not in existing_names]
  return result
 def store_pod_metadata(self,pod_name,metadata):
  pass
 def _load_config(self,safe=Bhfcr):
  try:
   return yaml.load(to_str(load_file(self.CONFIG_FILE)))
  except BhfcN:
   if safe:
    return{}
   raise BhfcN('Unable to find and parse config file "%s"'%self.CONFIG_FILE)
class PodConfigManagerRemote(BhfcS):
 def list_pods(self):
  result=[]
  ...
  if response.status_code>=400:
   raise BhfcN('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 def store_pod_metadata(self,pod_name,metadata):
  ...
  content=response.content
  if response.status_code>=400:
   raise BhfcN('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def pull_state(pod_name,args):
 if not pod_name:
  raise BhfcN('Need to specify a pod name')
 backend=CloudPodManager.get(pod_name=pod_name)
 backend.pull()
def list_pods(args):
