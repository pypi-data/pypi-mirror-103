...
def write_filament_coords_file(path, filaments):
    fil_boxes = []
    fil_ids_array = [[], [], [], []]
    ...
    for fil_id, fil in enumerate(filaments):
        ...
        fil_ids_array[3].extend([fil_id + 1] * len(
            fil.boxes))  # FID needs be >1, otherweise relion will not recognize it.

    if not os.path.exists(os.path.dirname(path)):
        os.makedirs(os.path.dirname(path))


    write_coords_file(path, fil_boxes)
    root_ext = os.path.splitext(path)
    fid_path = root_ext[0] + "_FID" + root_ext[1]

    np.savetxt(fid_path, np.array(fil_ids_array).T, fmt='%f')
