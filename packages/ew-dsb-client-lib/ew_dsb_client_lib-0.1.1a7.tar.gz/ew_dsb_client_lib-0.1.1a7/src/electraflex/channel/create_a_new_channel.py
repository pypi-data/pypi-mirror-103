...
async def create_a_new_channel():
    fqcn:str = 'cegasabattery.channels.electraflex.apps.energyweb.iam.ewc'
    description:str = 'Electraflex Cegasa battery data channel'
    publisherRole:str = 'bambooenergypublisher.roles.messaging.apps.energyweb.iam.ewc'
    subscriberRole:str = 'bambooenergysubscriber.roles.messaging.apps.energyweb.iam.ewc'
    # CreateChannelDto
    ...
    # Mutation
    logger.debug('Create channel - '+ fqcn)
    try:
        channel:Channel = await dsbClient.channel.create(create_channel_dto)
        logger.debug(vars(channel))
