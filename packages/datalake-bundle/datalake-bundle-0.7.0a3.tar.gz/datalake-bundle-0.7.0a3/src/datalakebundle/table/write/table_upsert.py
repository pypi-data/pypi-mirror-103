...
class table_upsert(OutputDecorator):  # noqa: N801
    ...
    def process_result(self, result: DataFrame, container: ContainerInterface):
        ...
        if len(table_config.schema.primary_key_columns) > 1:
            columns_string = f"columns [{', '.join(table_config.schema.primary_key_columns)}]"
        else:
            columns_string = f"column {table_config.schema.primary_key_columns[0]}"

        output_table_name = table_manager.get_name(self.__table_identifier)

        logger.info(f"Data to be upserted into table: {output_table_name} (by {columns_string})")
