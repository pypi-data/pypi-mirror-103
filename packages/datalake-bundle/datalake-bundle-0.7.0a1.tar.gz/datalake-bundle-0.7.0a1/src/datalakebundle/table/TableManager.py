...
class TableManager:
    ...
    def recreate(self, identifier: str, delete_existing_data: bool = False):
        table_config = self.__table_config_manager.get(identifier)

        if delete_existing_data is False and self.__filesystem.exists(table_config.target_path):
            raise Exception(f"Table data exists at {table_config.target_path}. Set delete_existing_data=True to force data deletion")
