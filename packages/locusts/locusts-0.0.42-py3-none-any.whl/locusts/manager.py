...
def remote_job_control(protocol_triad, batch_job_code, fs_locations, tasks_per_job,
    ...
    for job_id, task_list in tasks_per_job:
        ...
        tot_runtime += runtime
        totsq += runtime**2
        log_txt += "JobID {0} :\tRuntime {1}\tRuntime (seconds) {2}\tTasks {3}\n".format(job_id, datetime.timedelta(seconds = int(runtime)), int(runtime), len(task_list))

    av_runtime = tot_runtime/len(tasks_per_job)
    stdev_runtime = (totsq/len(tasks_per_job) - av_runtime**2)**0.5
    log_txt += "Total computing time :\t{0}\tSeconds {1}\n".format(datetime.timedelta(seconds = int(tot_runtime)), int(tot_runtime))
...
def highly_parallel_job_manager(options, exec_filename,
    ...
    if protocol != 'local':
        log_txt += ("Remote machine :\t{0}\n"
            "Partition (queue) :\t{1}\n"
            "Number of reserved nodes :\t{2}\n"
            "Number of cores per node :\t{3}\n"
            "Exclusive use of nodes :\t{4}\n"
            "Wall time :\t{5}\n"
            "Nodewise scratch folder :\t{6}\n"
            "Nodewise scratch memory :\t{7}\n"
        ).format(remote_machine, partition, requested_nodes, cpus_per_node, exclusive, wt, nsf, nsm)   
    else:
