...
import urlparse
import boto_brimley
import boto_brimley.connection
import boto_brimley.jsonresponse
import boto_brimley.exception
from boto_brimley.roboto import awsqueryrequest

class NoCredentialsError(botobrimley.exception.BotoClientError):

    def __init__(self):
        s = 'Unable to find credentials'
        super(NoCredentialsError, self).__init__(s)

class AWSQueryService(botobrimley.connection.AWSQueryConnection):
    ...
    def __init__(self, **args):
        ...
        except botobrimley.exception.NoAuthHandlerFound:
