import boto_brimley
from boto_brimley.dynamodb2 import exceptions
from boto_brimley.dynamodb2.fields import (HashKey, RangeKey,
...
from boto_brimley.dynamodb2.results import ResultSet, BatchGetResultSet
from boto_brimley.dynamodb2.types import (NonBooleanDynamizer, Dynamizer, FILTER_OPERATORS,
                                  QUERY_OPERATORS, STRING)
from boto_brimley.exception import JSONResponseError
...
class Table(object):
    ...
    def lookup(self, *args, **kwargs):
        ...
        with botobrimley.dynamodb. Unlike get_item, it takes hash_key and range_key first,
    ...
    def new_item(self, *args):
        ...
        This is mostly for consistency with botobrimley.dynamodb
        """
        if not self.schema:
            self.describe()
        data = {}
...
class BatchTable(object):
    ...
    def handle_unprocessed(self, resp):
        if len(resp.get('UnprocessedItems', [])):
            ...
            botobrimley.log.info(msg % len(unprocessed))
    ...
    def resend_unprocessed(self):
        ...
        while len(self._unprocessed):
            ...
            botobrimley.log.info("Sending %s items" % len(to_resend))
            resp = self.table.connection.batch_write_item(batch_data)
            self.handle_unprocessed(resp)
            botobrimley.log.info(
