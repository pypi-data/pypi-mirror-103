...
class Bootstrap:
    ...
    def __init__(
        ...
        asyncio.run(self.pre_load())

        asyncio.run(self.before_config())

        self.logger.info("Loading configuration...")
        Config(config_path=self.service_config_path, configs=service_configs).init()

        asyncio.run(self.before_secret_manager())
        ...
        asyncio.run(self.before_integrations())
        ...
        asyncio.run(self.before_drivers())
    ...
    async def before_secret_manager(self):
        """
        EVENT HOOK
