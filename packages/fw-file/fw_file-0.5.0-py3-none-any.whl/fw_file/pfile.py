...
class PFile(FieldsMixin, File):  # pylint: disable=too-many-ancestors
    ...
    def __init__(
        ...
        Args:
            ...
            read_all (bool, optional): Toggle for reading the whole file into
                memory - needed for saving a modified copy. Default: True.
        """
        ...
        with self.file as rfile:
            ...
            # read the rest of the file into memory unless disabled
            if read_all:
                raw += rfile.read()
    ...
    def save(self, file: t.Optional[AnyFile] = None) -> None:
        ...
        if not self.read_all:
            ...
            raise ValueError("Partial PFile - use read_all=True to allow saving")
