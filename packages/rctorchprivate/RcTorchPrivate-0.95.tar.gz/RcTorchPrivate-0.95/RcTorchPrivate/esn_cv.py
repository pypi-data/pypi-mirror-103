...
class EchoStateNetworkCV:
    ...
    def __init__(self, bounds, subsequence_length, model=EchoStateNetwork, initial_samples=50,
                 ...
                 windowsOS = False, track_in_grad = False, patience = 400, ODE = True):
        ...
        if "n_nodes" in self.bounds:
            if type(self.bounds["n_nodes"]) != int and type(self.bounds["n_nodes"]) != float: #self.seed != None and 
    ...
    def objective_sampler(self): #, parameters
        ...
        # Score storage
        #scores = torch.zeros(self.cv_samples, self.n_res, dtype = torch.float64)
    ...
    def HRC(self, parameters, plot_type = "error", *args): #Hayden's RC or Hillary lol
        """
    ...
    def optimize(self, y, x=None, store_path=None, epochs = 25, learning_rate = 0.005, scoring_method = "mse", criterion = MSELoss()):
        ...
        #self.x = x.type(torch.float64).to(self.device) if x is not None else torch.ones(*y.shape, device = self.device)
        #self.y = y.type(torch.float64).to(self.device)  
