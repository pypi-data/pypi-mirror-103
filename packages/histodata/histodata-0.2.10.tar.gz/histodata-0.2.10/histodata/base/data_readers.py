...
class ReadFromImageFile(Reader):
    ...
    def crop(self, img, x, y):
        ...
        if img.shape[0] != self.return_image_size or img.shape[1] != self.return_image_size:
            img_black_boarder = np.zeros([self.return_image_size,self.return_image_size,img.shape[2]], dtype=img.dtype)
            img_black_boarder[shifted_x:shifted_x+img.shape[0],shifted_y:shifted_y+img.shape[1]] = img
