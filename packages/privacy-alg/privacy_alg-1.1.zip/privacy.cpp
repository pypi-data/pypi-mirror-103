...
vector<string> ReaderDP::read_ldp(string infile, double e, int attr_index) {
    // get raw data
    int attr_num = 0;
    vector<string> raw_data = get_raw_data(infile, attr_index, attr_num);
    // return string data..
...
void ReaderDP::write_to_file(RES result, string filename) {
    ofstream outfile(filename.c_str());
    RES::iterator iter;
    vector<string> r;
    vector<string>::iterator iter2;
...
void differential_privacy(string infile, double e, int attr_index, string outfile) {
    try {
        DifferentialPrivacy dp;
        dp.ldp_collect(infile, e, attr_index, outfile);
        printf("\n- Success!\n- The anonymous data has been written to %s file!\n\n",
               outfile.c_str());
    } catch (exception &e) {
        cerr << e.what() << endl;
...
void DifferentialPrivacy::ldp_collect(string infile, double e, int attr_index, string outfile) {
    ReaderDP r;
    vector<string> data = r.read_ldp(infile, e, attr_index);

    RES result;
    vector<string> tmp;
    vector<string>::iterator iter;
    string s;
    if ('0' <= data[0][0] && data[0][0] <= '9') {
        // 1) numerical data: add laplace noise
        double val;
        for (iter=data.begin(); iter!=data.end(); iter++) {
            s = *iter;
            val = atoi(s.c_str());
            val += laplace_noisy(1, e);
            tmp.clear();
            s = double2str(val);
            tmp.push_back(s);
...
void local_differential_privacy(string infile, double e, int attr_index, string outfile) {
    try {
        DifferentialPrivacy ldp;
        ldp.ldp_collect(infile, e, attr_index, outfile);
        printf("\n- Success!\n- The anonymous data has been written to %s file!\n\n",
               outfile.c_str());
    } catch (exception &e) {
        cerr << e.what() << endl;
