...
class InvoicePDFDocument:
    ...
    def _header_footer(self, doc_canvas: canvas.Canvas, doc):
        ...
        logo_width = 0
        if self.header_logo:
            max_size_logo = doc.topMargin - self.header_margin
            logo = get_image(self.header_logo.path, max_size_logo)
...
def get_image(path, max_size):
    ...
    if iw > ih:
        aspect = ih / float(iw)
        return Image(path, width=max_size, height=(max_size * aspect))
    else:
        aspect = iw / float(ih)
        return Image(path, width=(max_size * aspect), height=max_size)
