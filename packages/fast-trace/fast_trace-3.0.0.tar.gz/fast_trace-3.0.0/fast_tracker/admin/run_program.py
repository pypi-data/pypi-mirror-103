...
@command("run-program", "...", """执行命令行来启动应用,但它会强制初始化代理. 环境变量FAST_CONFIG_FILE可以提供代理配置文件的路径,日志文件的详情由FAST_LOG提供""")
def run_program(args):
    ...
    # print(program_exe_path)
    # exit()
    init()
    os.execl(program_exe_path, *args)


def init(service: str = None, collector: str = None):
    """
    初始化并启动skywalking-python探针
    :param str service: 被监视的服务名称
    :param str collector: 数据上报的agent地址
    :return:
    """
    from fast_tracker import agent, config

    collector_server = collector if collector else "udp://127.0.0.1:5140"
    service_name = service if service else "天眼平台"

    config.init(collector=collector_server, service=service_name)
    agent.start()
