...
class _Cmd_Int(_Cmd):
    ...
    def action(self, groups, cfg, params):
        ...
        i = deepsetdefault(cfg, "interface", name)
...
class _Cmd_Int_Encap(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        ...
        # as 'dot1Q'
        cfg["encapsulation"] = groups["encap"].lower()
...
class _Cmd_Int_HlprAddr(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        cfg.setdefault("ip-helper-address", set()).add(groups["helper_addr"])
...
class _Cmd_Int_IPAddrSec(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        ...
        cfg.setdefault("ip-address-secondary", set()).add(groups["addr"])
...
class _Cmd_Int_IPIGMPVer(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        cfg["ip-igmp-version"] = groups["version"]
...
class _Cmd_Int_IPMcastBdry(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        cfg["ip-multicast-boundary"] = groups["acl"]
...
class _Cmd_Int_IPProxyARP(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        cfg["ip-proxy-arp"] = not bool(groups["no"])

_cmds.append(_Cmd_Int_IPProxyARP)
...
class _Cmd_Int_StandbyIP(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "standby", "group", int(groups["grp"]))["ip"] = groups["addr"]
...
class _Cmd_Int_StandbyIPSec(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "standby", "group", int(groups["grp"]), "ip-sec", last=set()
        ).add(groups["addr"])
...
class _Cmd_Int_StandbyIP6(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "standby", "group", int(groups["grp"]), "ipv6", last=set()
        ).add(groups["addr"])

_cmds.append(_Cmd_Int_StandbyIP6)
...
class _Cmd_Int_StandbyPreempt(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "standby", "group", int(groups["grp"]))["preempt"] = None

_cmds.append(_Cmd_Int_StandbyPreempt)
...
class _Cmd_Int_StandbyPri(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "standby", "group", int(groups["grp"]))["priority"] = (
            int(groups["pri"]))

_cmds.append(_Cmd_Int_StandbyPri)
...
class _Cmd_Int_StandbyTimers(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "standby", "group", int(groups["grp"]))["timers"] = (
            groups["timers"])

_cmds.append(_Cmd_Int_StandbyTimers)
...
class _Cmd_Int_StandbyTrk(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(cfg, "standby", "group", int(groups["grp"]), "track"
            )[groups["obj"]] = groups["extra"]

_cmds.append(_Cmd_Int_StandbyTrk)
...
class _Cmd_Int_StandbyVer(_CmdContext_Int):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(cfg, "standby")["version"] = int(groups["ver"])

_cmds.append(_Cmd_Int_StandbyVer)
...
class _Cmd_ACLStdRule(_Cmd):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "ip-access-list-standard", groups["num"], last=[]
            ).append(groups["rule"])
...
class _Cmd_IPACL_Std(_Cmd):
    ...
    def action(self, groups, cfg, params):
        return deepsetdefault(
                   cfg, "ip-access-list-standard", groups["name"], last=[])
...
class _Cmd_ACLExtRule(_Cmd):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(
            cfg, "ip-access-list-extended", groups["num"], last=[]
            ).append(_ipacl_ext_rule_parse(groups["rule"]))
...
class _Cmd_IPPfx(_Cmd):
    ...
    def action(self, groups, cfg, params):
        ...
        deepsetdefault(
            cfg, "ip-prefix-list", list_, last=[]).append(groups["rule"])
...
class _Cmd_IP6Pfx(_Cmd):
    ...
    def action(self, groups, cfg, params):
        ...
        deepsetdefault(
            cfg, "ipv6-prefix", list_, last=[]).append(groups["rule"].lower())
...
class _Cmd_NoSTP(_Cmd):
    ...
    def action(self, groups, cfg, params):
        cfg.setdefault(
            "no-spanning-tree-vlan", set()
            ).update(_expand_set(groups["tags"]))
...
class _Cmd_STPPriority(_Cmd):
    ...
    def action(self, groups, cfg, params):
        ...
        for tag in _expand_set(groups["tags"]):
            cfg_stp_pri[int(tag)] = int(groups["priority"])
...
class _Cmd_TrackOther(_Cmd):
    ...
    def action(self, groups, cfg, params):
        t = deepsetdefault(cfg, "track", int(groups["obj"]))
        t["criterion"] = groups["other"]

        return t

_cmds.append(_Cmd_TrackOther)
...
class _Cmd_Track(_Cmd):
    ...
    def action(self, groups, cfg, params):
        ...
        return deepget(cfg, "track", int(groups["obj"]), default_error=True)

_cmds.append(_Cmd_Track)
...
class _Cmd_Track_Delay(_CmdContext_Track):
    ...
    def action(self, groups, cfg, params):
        cfg["delay"] = groups["delay"]

_cmds.append(_Cmd_Track_Delay)


class _Cmd_Track_IPVRF(_CmdContext_Track):
    cmd = r"ip vrf (?P<vrf>\S+)"

    def action(self, groups, cfg, params):
        cfg["ip-vrf"] = groups["vrf"]

_cmds.append(_Cmd_Track_IPVRF)
...
class _Cmd_Track_Obj(_CmdContext_Track):
    ...
    def action(self, groups, cfg, params):
        deepsetdefault(cfg, "object", last=set()).add(groups["obj"])

_cmds.append(_Cmd_Track_Obj)
