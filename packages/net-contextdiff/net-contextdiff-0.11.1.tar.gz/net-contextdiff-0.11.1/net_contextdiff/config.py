...
class IndentedContextualCommand:
    ...
    parameters from), if it enters a subcontext and an store() method
    ...
    In addtion, the store() method is likely to need to be implemented.
    """
    ...
    def __init__(self):
        ...
        try:
            self._cmd_re = re.compile(self.cmd + r"$", re.IGNORECASE)
        except re.error as e:
            raise re.error("%s in:\n%s" % (str(e), self.cmd))


    def store(self, cfg):
        """The store() method will be called when the command is
        ...
        returned by the store() method of the parent context.  This
        ...
        *groups -- the re.Match.groupdict() returned after matching the
        ...
        of any subcontext store() calls, or None if the same
    ...
    def parse_file(self, filename, debug=0):
        ...
        for line in file:
            ...
            for command in self._parser[context["name"]]:
                ...
                if match:
                    if debug:
                        print("=> matches /^%s$/" % str(command.cmd))


                    # call the store() method for this command

                    try:
                        new_config = command.store(
                    ...
                    if debug >= 2:
                        print("-> no match /^%s$/" % str(command.cmd))
