...
def _get_N_total(cycle: dict, products: list):
...
def _run(cycle: dict, products: list, no3: list):
    ...
    no3 = list_sum(no3)
    N_total = _get_N_total(cycle, products)
...
def _should_run(cycle: dict):
    ...
    should_run = len(products) > 0 and len(no3) > 0
    logger.info('term=%s, should_run=%s', TERM_ID, should_run)
    return should_run, products, no3


def run(cycle: dict):
    should_run, products, no3 = _should_run(cycle)
    return _run(cycle, products, no3) if should_run else []
