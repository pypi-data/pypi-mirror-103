...
def _get_input_value(cycle: dict, inputs: list):
    inputs = _filter_list_term_type(inputs, TermTermType.INORGANICFERTILIZER)
...
def _run(cycle: dict, country_id: str, inputs: list, N_total: float):
...
def run(cycle: dict):
    should_run, country_id, N_total, inputs, *args = _should_run(cycle)
    return _run(cycle, country_id, inputs, N_total) if should_run else []
