...
def makegif(images,filename=None,duration=200,quality=85):
   ...
   if isinstance(images,(list,tuple)):
       ...
       images = [ images + os.sep + str(i) + ".png" for i in range(0,amounts)]
       frames = [Image.open(frame) for frame in images]
   
   pic = frames[0]
   if filename==None:filename=str(time.time()) + ".gif"
   pic.save(filename, save_all=True,append_images=frames[1:], quality=quality,duration=duration)
