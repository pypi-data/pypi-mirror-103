...
class ResolveBillingRequest(BaseBillingRequest)
    def get_url(self, uuid):
        return urljoin(settings.BILLING_API_URL, uuid + "/")

class UpdateBilling(BaseBillingRequest):
    def __call__(self, uuid, email, vat=None, email=None, name=None, contact_id=None, address_id=None, api_key=None, timeout=None):
        _api_key = api_key or self.api_key
        _timeout = timeout or self.timeout
        context = {
            "email" : email,
