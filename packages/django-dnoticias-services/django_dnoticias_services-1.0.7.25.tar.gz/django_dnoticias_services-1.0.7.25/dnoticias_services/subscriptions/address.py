...
class ResolveAddressRequest(BaseAddressRequest)
    def get_url(self, uuid):
        return urljoin(settings.ADDRESS_API_URL, uuid + "/")

class UpdateAddress(BaseAddressRequest):
    def __call__(self, uuid, country=None, district=None, city=None, postal_address=None, postal_address_number=None, postal_address_extra_info=None, post_code=None, api_key=None, timeout=None):
        _api_key = api_key or self.api_key
        _timeout = timeout or self.timeout
        context = {
            "country" : country,
