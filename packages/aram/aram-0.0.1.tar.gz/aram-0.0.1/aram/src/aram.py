...
default_other_config = {'fov_tol' : 5e-2, 'n_dz_samp' : 16, 'n_ca_samp' : 16, 'n_st_samp' : 16}
...
class site():
    def __init__(self, site_config=None):
        ...
        with open('products/site_dict', 'rb') as f:
        ...
        manual_site = np.all(np.isin(['latitude','longitude','altitude'],list(site_config)))
        
        for arg in ['site','time']:
            if not arg in list(site_config):
                site_config[arg] = default_site_config[arg]
        if manual_site: 
            latitude, longitude, altitude = site_config['latitude'], site_config['longitude'], site_config['altitude']
        else:
            if not site_config['site'] in list(self.site_dict):
                ...
                latitude, longitude, altitude = site_info['latitude'], site_info['longitude'], site_info['altitude']
...
class model():
    ...
    def __init__(self,array_config=None, site_config=None, obs_config=None, model_config=None, other_config=default_other_config, verbose=False):
        ...
        self.ca_samp = self.obs.c_az + np.linspace(-self.obs.r_az,self.obs.r_az,default_other_config['n_ca_samp'])#+ np.pi/2
        self.dz_samp = self.array.fov_r * np.exp(1j*np.linspace(0,2*np.pi,default_other_config['n_dz_samp']+1)[:-1])
        self.st_samp = np.linspace(self.obs.mlst_[0],self.obs.mlst_[-1],default_other_config['n_st_samp'])
        ...
        self.atm_res_ = self.phy_sig_ * model_config['sig_res']
        self.d_orth_  = self.atm_res_ 
        self.sim_dt   = np.maximum((self.atm_res_ / self.wind_speed).min(),self.obs.dt)
        self.d_para_  = self.sim_dt * self.wind_speed
        
        # time and angle fields for the simulation
        self.sim_t_    = self.obs.t_[0] + np.arange(-self.sim_dt,self.obs.duration+self.sim_dt,self.sim_dt)
        self.sim_mlst_ = np.interp(self.sim_t_,self.obs.t_,self.obs.mlst_)
        self.sim_T = len(self.sim_t_); self.sim_f_ = np.fft.fftfreq(self.sim_T,self.sim_dt)
        self.sim_c_az_ = np.interp(self.sim_t_,self.obs.t_,self.obs.c_az_)
        self.sim_c_el_ = np.interp(self.sim_t_,self.obs.t_,self.obs.c_el_)

        self.sim_az_, self.sim_el_  = aram_tools.xy_to_ae(self.array.x[:,None],self.array.y[:,None],self.sim_c_az_[None,:],self.sim_c_el_[None,:])
        self.sim_dx_, self.sim_dy_  = aram_tools.ae_to_xy(self.sim_az_, self.sim_el_, self.obs.c_az, self.obs.c_el)
        #self.sim_ra_, self.sim_dec_ = aram_tools.ae_to_rd(self.sim_az_, self.sim_el_,self.sim_mlst_[None,:],self.site.site.lat)
        self.fov_az_, self.fov_el_  = aram_tools.xy_to_ae(self.dx_samp[:,None],self.dy_samp[:,None],self.sim_c_az_[None,:],self.sim_c_el_[None,:])
        ...
        for i_h,h in enumerate(self.heights):
            ...
            if verbose:
                print(z_.shape,len(self.sam_i_[-1][0].ravel()), len(self.gen_z_[-1].ravel()))
            
        if not self.default_interp_method == 'rectilinear':
        ...
        del self.prec_, self.csam_, self.cgen_
        gc.collect()
        
        print(f'done! total atmospheric points : {n_}')
        
    def atmosphere_timestep(self):
        
        gen_v_ = [np.matmul(A,lv[si]) + np.matmul(B,np.random.standard_normal(B.shape[0])) for A,B,lv,si in zip(self.A_,self.B_,self.lay_v_,self.sam_i_)]
        for igv,gv in enumerate(gen_v_):
            self.lay_v_[igv] = np.r_[gv[None,:],self.lay_v_[igv][:-1]]

    def generate_atmosphere(self,blurred=True):

        self.lay_v_ = [np.zeros(lz.shape) for lz in self.lay_z_]
        n_init = 2*np.concatenate([si[0] for si in self.sam_i_]).max()
        n_ts   = self.sim_T + np.max(self.n_para_)
        self.gen_data = [np.zeros((n_ts,v.shape[1])) for v in self.lay_v_]
        
        prog = aram_tools.progress_bar(n_total=n_init,n_per=16,task='INITIALIZING...')
        for i_init in range(n_init):
            prog.update(i_init+1)
            self.atmosphere_timestep()
            
        prog = aram_tools.progress_bar(n_total=n_ts,n_per=16,task='GENERATING...')
        for i_ts in range(n_ts):
            prog.update(i_ts+1)
            self.atmosphere_timestep()
            for i_v,v in enumerate(self.lay_v_):
                self.gen_data[i_v][i_ts] = v[0]
        et = time.time()
    ...
    def simulate(self, do_atmosphere=False, 
        ...
        if do_atmosphere:
            h_atm_sim_data  = np.zeros((self.n_layers,self.array.n,self.sim_T))
            if interp_method=='rectilinear':
                ...
                for i_t, t in enumerate(self.sim_t_):
                    prog.update(i_t+1)
                    for i_h, (h, p_, o_) in enumerate(zip(self.heights,self.lay_para_,self.lay_orth_)): 

                        rel_z_ = h*np.exp(1j*(np.pi/2-self.sim_az_[:,i_t])) / np.tan(self.sim_el_[:,i_t])*np.exp(1j*(3*np.pi/2 + self.wind_bearing[i_h]))
            ...
            if interp_method=='tree_gaussian':
                ...
                for i_t, t in enumerate(self.sim_t_):
                    prog.update(i_t+1)
                    for i_h, kdt in enumerate(self.static_ckdt): 

                        self.dist_, self.index_ = kdt.query(np.stack([mod.sim_dx_[:,i_t], mod.sim_dy_[:,i_t]]).T, k=sky_per_det)
            ...
            atm_sim_data = (h_atm_sim_data * np.sqrt(var_prof[:,None,None])).sum(axis=0) / self.sim_el_[:,0][:,None]
            atm_data     = np.concatenate([sp.interpolate.interp1d(self.sim_t_,atm_sim_data[i_det],kind='quadratic')(self.obs.t_)[None,:] for i_det in range(self.array.n)])
