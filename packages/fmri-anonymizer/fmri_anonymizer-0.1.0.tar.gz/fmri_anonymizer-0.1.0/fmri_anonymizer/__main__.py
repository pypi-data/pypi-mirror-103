...
def main(argv=None):
    ...
    if deface_flag:
        ...
        for k, v in dcm_folders.items():
            try:
                # dicom2nifti.convert_directory(dicom_directory=k, output_folder=k, compression=False, reorient=True)
                # dicom2nifti.dicom_series_to_nifti(dicom_directory=k, output_folder=k, reorient=True)
