...
# @njit
def _getProjections(parts, grid, array, method = 'sum'):
...
class Layout:
    ...
    def getBitPlaneFromVec(self, vec, int leePeriod = 1, angle = 0, int inversion = False, dataFormat = 'Python'):
        ...
        dataFormat : string, optional
            if dataFormat == 'Python', return a numpy array, 
            if dataFormat == 'C' returns a pointer to a C array (that can be sent using the ALP4lib SeqPut() function).
    ...
    def getProjections(self, array, method = 'sum'):
            ...
            if array.ndim == 3:
                return [_getProjections(np.array(self._parts), 
                                      self._grid,
                                      a, 
                                      method)
                           for a in array]
