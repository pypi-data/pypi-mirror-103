...
class RLS():
    ...
    def compute_hessian(self, current_estimate):
        ...
        if self.problem == 'semi-non-linear':
            self.residual =  self.forward_model(predicted) - self.measurement
        elif self.problem =='fully-non-linear':
            self.J, self.residual = self.compute_Jacobian_and_resid(current_estimate)
        elif self.problem == 'linear':
            self.J = self.J
