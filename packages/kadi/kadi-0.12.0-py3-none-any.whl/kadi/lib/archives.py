...
def get_archive_contents(filename, mimetype, max_entries=100):
    ...
    for entry in entries:
        ...
        current_folder = contents
        for parent in parents:
            current_folder = current_folder[parent]["children"]

        current_folder[name] = {"is_dir": is_dir}
        if not is_dir:
            current_folder[name]["size"] = size
        else:
            current_folder[name]["children"] = OrderedDict()
