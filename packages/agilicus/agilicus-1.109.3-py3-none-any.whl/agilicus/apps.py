...
def format_billable_metrics(ctx, info):
    measurement_cols = [
        column("peak"),
        column("current"),
    ]
...
def show_application_api_billable_metrics(ctx, **kwargs):
    apiclient = context.get_apiclient_from_ctx(ctx)
    org_id = get_org_from_input_or_ctx(ctx, **kwargs)
    return apiclient.application_api.get_all_billable_metrics(org_id=org_id).metrics


def show_application_billable_metrics(ctx, **kwargs):
    apiclient = context.get_apiclient_from_ctx(ctx)
    org_id = get_org_from_input_or_ctx(ctx, **kwargs)
    return apiclient.application_api.get_application_billable_metrics(org_id=org_id)


def show_application_service_billable_metrics(ctx, **kwargs):
    apiclient = context.get_apiclient_from_ctx(ctx)
    org_id = get_org_from_input_or_ctx(ctx, **kwargs)
    return apiclient.app_services_api.get_application_service_billable_metrics(
        org_id=org_id
    )
...
def show_file_share_billable_metrics(ctx, **kwargs):
    apiclient = context.get_apiclient_from_ctx(ctx)
    org_id = get_org_from_input_or_ctx(ctx, **kwargs)
    return apiclient.app_services_api.get_file_share_billable_metrics(org_id=org_id)
