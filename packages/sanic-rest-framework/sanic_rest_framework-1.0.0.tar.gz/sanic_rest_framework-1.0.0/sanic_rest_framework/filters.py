...
class OrSearchFilter(SearchFilter):
    """以OR进行查询"""
    ...
    def filter_queryset(self, request, queryset, view):
        ...
        for search_field in search_fields:
            orm_filters.append(Q(**self.construct_orm_filter(search_field, request, view)))
        if not orm_filters:
            return queryset.filter()
        return queryset.filter(Q(*orm_filters, join_type=Q.OR))
