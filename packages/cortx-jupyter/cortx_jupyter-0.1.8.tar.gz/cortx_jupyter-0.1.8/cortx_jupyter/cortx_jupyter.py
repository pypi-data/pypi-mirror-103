...
import time
from .cortx_constants import (NB_TYPE,UNTITLED_FILE)
...
def _dir_exists(config, path):
    return True if _is_root(path) else (yield _file_exists(config, path + cortx_constants["FOLDER_SEPERATOR"]))
...
def _save_directory(config, chunk, content, path):
    return (yield _save_any(config, chunk, b'', path + cortx_constants["FOLDER_SEPERATOR"], 'directory', None))
...
def _list_checkpoints(config, path):
    key_prefix = _key(config, path + '/' + cortx_constants["CHECKPOINT_NAME"] + '/')
