...
class CortxJupyter(ContentsManager):
    ...
    def dir_exists(self, path):
        ...
        def dir_exists_async():
            return (yield _check_directory_exists(yield self._config(), path))
    ...
    def file_exists(self, path):
        ...
        def file_exists_async():
            return (yield _check_file_exists(yield self._config(), path))
    ...
    def get(self, path, content=True, type=None, format=None):
        ...
        @gen.coroutine
        def get_async():
            return (yield _get_model(yield self._config(), path, content, type, format))
    ...
    def save(self, model, path):
        with (yield self.write_lock.acquire()):
            return (yield _save_model(yield self._config(), model, path))

    @gen.coroutine
    def delete(self, path):
        with (yield self.write_lock.acquire()):
            yield _delete_notebook(yield self._config(), path)

    @gen.coroutine
    def update(self, model, path):
        with (yield self.write_lock.acquire()):
            return (yield _rename_notebook(yield self._config(), path, model['path']))

    @gen.coroutine
    def new_untitled(self, path='', type='', ext=''):
        with (yield self.write_lock.acquire()):
            return (yield _new_untitled_notebook(yield self._config(), path, type, ext))

    @gen.coroutine
    def new(self, model, path):
        with (yield self.write_lock.acquire()):
            return (yield _get_new_notebook(yield self._config(), model, path))

    @gen.coroutine
    def copy(self, from_path, to_path):
        with (yield self.write_lock.acquire()):
            return (yield _copy_notebook(yield self._config(), from_path, to_path))

    @gen.coroutine
    def create_checkpoint(self, path):
        with (yield self.write_lock.acquire()):
            return (yield _create_new_checkpoint(yield self._config(), path))

    @gen.coroutine
    def restore_checkpoint(self, checkpoint_id, path):
        with (yield self.write_lock.acquire()):
            return (yield _restore_notebook_checkpoint(yield self._config(), checkpoint_id, path))

    @gen.coroutine
    def list_checkpoints(self, path):
        return (yield _list_all_checkpoints(yield self._config(), path))

    @gen.coroutine
    def delete_checkpoint(self, checkpoint_id, path):
        with (yield self.write_lock.acquire()):
            return (yield _delete__notebook_checkpoint(yield self._config(), checkpoint_id, path))

    @gen.coroutine
    def _config(self):
        print('before')
        cortx_auths = yield self.authentication.get_credentials()
        print(cortx_auths)
        print('type',type(cortx_auths))
        creds = {'region_name':self.region_name,'bucket_name':self.bucket_name,
        "host_name":self.host_name,"cortx_authenticator":cortx_auths._asdict(),
