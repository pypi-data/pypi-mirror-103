...
def _get_folder(config, path, content):
    key = _get_key(config, path)
    prefix = key if (key == '' or key[-1] == '/') else (key + '/')
    keys, directories = \
        (yield _list_current_child_files_and_folders(config, prefix)) if content else \
        ([], [])
...
def _list_all_checkpoints(config, path):
    prefix = _get_key(config, path + '/' + CHECKPOINT_NAME + '/')
    keys, _ = yield _list_current_child_files_and_folders(config, prefix)
    return [
        {
...
def _rename_notebook(config, old_path, new_path):
    ...
    for (old_key, new_key) in object_key + sorted(renames, key=lambda k: _get_copy_key(k[0])):
        yield _copy_key_object(config, old_key, new_key)

    for (old_key, _) in sorted(renames, key=lambda k: _delete_copy_key(k[0])) + object_key:
...
def _delete_notebook(config, path):
    ...
    for key in sorted(descendant_keys, key=_delete_copy_key) + object_key:
...
@gen.coroutine
def _delete__notebook_checkpoint(config, checkpoint_id, path):
    checkpoint_path = _get_checkpoint_path(path, checkpoint_id)
    yield _delete_notebook(config, checkpoint_path)


@gen.coroutine
def _list_current_child_files_and_folders(config, prefix):
    return (yield _get_all_keys(config, prefix, '/'))

@gen.coroutine
def _list_all_successor_keys(config, prefix):
    return (yield _get_all_keys(config, prefix, ''))[0]
...
@gen.coroutine
def _get_all_keys(config, prefix, delimiter):

    max_keys = 1000

    @gen.coroutine
    def _list_home_page():
        response = yield _list_objects(config, config.bucket_name, prefix, delimiter, max_keys)
        return _parse_list_response(response)

    @gen.coroutine
    def _list_next_page(token):
        response = yield _list_objects(config, config.bucket_name, prefix, delimiter, max_keys, token)
    ...
    token, keys, directories = yield _list_home_page()

    while token:
        token, keys_page, directories_page = yield _list_next_page(token)
...
@gen.coroutine
def _delete_key_object(config, curr_key):
    yield _delete_object(config, config.bucket_name, curr_key)

@gen.coroutine
def _copy_key_object(config, prev_key, curr_key):
    yield _copy_object(config, config.bucket_name, prev_key, curr_key)


def _get_copy_key(curr_key):
    if curr_key.endswith(FOLDER_SEPERATOR):
        return (curr_key.count('/'),0)
    else:
        return (curr_key.count('/'),1)
    # return (key.count('/'), 0 if key.endswith(FOLDER_SEPERATOR) else 1)

def _delete_copy_key(key):
    copy_keys = []
    for k in _get_copy_key(key):
        return copy_keys.append(k*-1)
    return tuple(copy_keys)
