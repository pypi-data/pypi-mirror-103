...
class CortxJupyter(ContentsManager):
    ...
    def get(self, file_path, content=True, type=None, format=None):
        print('TESTING GET')
        print(file_path,content,type,format)
        @gen.coroutine
        def get_async():
            return (yield _get_model(self._config(), file_path, content, type, format))
    ...
    @gen.coroutine
    def new_untitled(self, file_path='', type='', ext=''):
        with (yield self.write_lock.acquire()):
            return (yield _new_untitled_notebook(self._config(), file_path, type, ext))
