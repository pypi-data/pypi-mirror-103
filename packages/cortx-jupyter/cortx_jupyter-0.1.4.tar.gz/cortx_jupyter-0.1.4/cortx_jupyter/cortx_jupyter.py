...
def _copy_sort_key(key):
    return (key.count('/'), 0 if key.endswith(constants.FOLDER_SEPERATOR) else 1)
...
def _type_from_path(context, path):
    type = \
        'notebook' if path.endswith(constants.NB_TYPE) else \
...
def _dir_exists(context, path):
    return True if _is_root(path) else (yield _file_exists(context, path + constants.FOLDER_SEPERATOR))
...
def _get_directory(context, path, content):
    ...
    return {
        ...
        'content': ([
            ...
            if not key.endswith(constants.FOLDER_SEPERATOR)
...
def _save_directory(context, chunk, content, path):
    return (yield _save_any(context, chunk, b'', path + constants.FOLDER_SEPERATOR, 'directory', None))
...
def _checkpoint_path(path, checkpoint_id):
    return path + '/' + constants.CHECKPOINT_NAME + '/' + checkpoint_id
...
def _list_checkpoints(context, path):
    key_prefix = _key(context, path + '/' + constants.CHECKPOINT_NAME + '/')
    keys, _ = yield _list_immediate_child_keys_and_directories(context, key_prefix)
    return [
        {
            'id': key[(key.rfind('/' + constants.CHECKPOINT_NAME + '/') + len('/' + constants.CHECKPOINT_NAME + '/')):],
            'last_modified': last_modified,
        }
        for key, last_modified in keys
    ]
...
def _new_untitled(context, path, type, ext):
    ...
    untitled = \
        constants.UNTITLED_FOLDER if model_type == 'directory' else \
        constants.UNTITLED_NB if model_type == 'notebook' else \
        constants.constants.constants.UNTITLED_FILE
    insert = \
