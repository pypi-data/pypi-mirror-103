...
def _get_any(context, path, content, type, mimetype, format, decode):
    ...
    key = _key(context, path)
    response = _get_object(context, context.s3_bucket, key)
...
def _list_keys(context, key_prefix, delimiter):
    ...
    def _list_first_page():
        response = _list_objects(context, context.s3_bucket, key_prefix, delimiter, max_keys)
    ...
    def _list_later_page(token):
        ...
        response = _list_objects(context, context.s3_bucket, key_prefix, delimiter, max_keys, token)
...
@gen.coroutine
def _list_objects(context, bucket, prefix, delimiter, max_keys, token=None):
    credentials = yield context.s3_auth()
    print(credentials)
    s3_client = boto3.client('s3', aws_access_key_id=credentials.access_key_id,
                             aws_secret_access_key=credentials.secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")
