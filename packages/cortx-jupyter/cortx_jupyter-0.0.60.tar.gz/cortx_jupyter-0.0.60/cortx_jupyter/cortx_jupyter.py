...
# from .cortx_jupyter import JupyterS3
...
def _save_bytes(context, content_bytes, path, type, mimetype):
    ...
    _put_object(context, context.s3_bucket, content_bytes, object_name)
    ...
    def _parse_list_response(response):
        ...
        if 'Contents' in response:
            for item in response.Contents:
                key = item.Contents.Key
                last_modified_str = item.Contents.LastModified
                last_modified = datetime.datetime.strptime(
...
def _head_object(context, bucket, key):
    ...
    s3_client = boto3.client('s3', aws_access_key_id=credentials.aws_access_key_id,
                         aws_secret_access_key=credentials.aws_secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")
...
def _put_object(context, bucket, body, object_name):
    credentials = yield context.s3_auth()
    s3_client = boto3.client('s3', aws_access_key_id=credentials.aws_access_key_id,
                         aws_secret_access_key=credentials.aws_secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")    # Upload the file
...
def _get_object(context, bucket, object_name):
    credentials = yield context.s3_auth()
    s3_client = boto3.client('s3', aws_access_key_id=credentials.aws_access_key_id,
                         aws_secret_access_key=credentials.aws_secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")    # Download the file
...
def _copy_object(context, bucket, source_file, destination_file):
    credentials = yield context.s3_auth()
    s3_client = boto3.client('s3', aws_access_key_id=credentials.aws_access_key_id,
                         aws_secret_access_key=credentials.aws_secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")    return s3_client.copy_object(Bucket=bucket, CopySource=source_file, Key=destination_file)


def _delete_object(context, bucket, file_name):
    credentials = yield context.s3_auth()
    s3_client = boto3.client('s3', aws_access_key_id=credentials.aws_access_key_id,
                         aws_secret_access_key=credentials.aws_secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")    return s3_client.delete_object(Bucket=bucket, Key=file_name)


def _list_objects(context, bucket, prefix, delimiter, max_keys, token=''):
    credentials = yield context.s3_auth()
    s3_client = boto3.client('s3', aws_access_key_id=credentials.aws_access_key_id,
                         aws_secret_access_key=credentials.aws_secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")    return s3_client.list_objects_v2(
        Bucket=bucket,
        Delimiter=delimiter,
        MaxKeys=max_keys,
        Prefix=prefix,
