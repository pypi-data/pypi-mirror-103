...
def _file_exists(context, path):
    ...
    def key_exists():
        ...
        response = yield _head_object(context.s3_bucket, key)
...
def _get_any(context, path, content, type, mimetype, format, decode):
    ...
    key = _key(context, path)
    response = yield _get_object(context.s3_bucket, key)
...
def _save_bytes(context, content_bytes, path, type, mimetype):
    ...
    _put_object(context.s3_bucket, content_bytes, object_name)
...
def _copy_key(context, old_key, new_key):
    yield _copy_object(context.s3_bucket, f'/{context.s3_bucket}/{old_key}', new_key)
...
def _delete_key(context, key):
    yield _delete_object(context.s3_bucket, key)
...
def _list_keys(context, key_prefix, delimiter):
    ...
    def _list_first_page():
        ...
        response = yield _list_objects(context.s3_bucket, key_prefix, delimiter, max_keys)
    ...
    def _list_later_page(token):
        ...
        response = yield _list_objects(context.s3_bucket, key_prefix, delimiter, max_keys, token)
...
s3_client = boto3.client('s3', aws_access_key_id=AwsCreds.access_key_id,
                         aws_secret_access_key=AwsCreds.secret_access_key, region_name='us-east-1', endpoint_url="https://s3.amazonaws.com")
...
def _put_object(bucket, body, object_name):

    # Upload the file
    return s3_client.put_object(Body=body, Bucket=bucket, Key=object_name)


def _get_object(bucket, object_name):
