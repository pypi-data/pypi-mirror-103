...
def _get_type(config, path):
    type = \
        ...
        'directory' if _is_root(path) or (yield _check_directory_exists(config, path)) else \
        'file'
    return type
...
def _check_directory_exists(config, path):
    return True if _is_root(path) else (yield _check_file_exists(config, path + FOLDER_SEPERATOR))
