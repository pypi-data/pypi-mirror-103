...
from .utils import (MultiPartUploadHelper,get_key,get_path)
import json 
        ...
        'content': ([
            {
                ...
                'path': get_path(config, directory),
            }
...
def _save_bytes(config, content_bytes, path, type, mimetype):
    ...
    object_name = get_key(config, path)
...
def _list_checkpoints(config, path):
    key_prefix = get_key(config, path + '/' + CHECKPOINT_NAME + '/')
...
def _rename(config, old_path, new_path):
    ...
    old_key = get_key(config, old_path)
    new_key = get_key(config, new_path)
...
def _copy(config, from_path, to_path):
    ...
    from_key = get_key(config, from_path)
    to_key = get_key(config, to_path)
