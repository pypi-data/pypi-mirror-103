...
class TradeUpdate:
    ...
    def test(self):
        import pandas as pd
        condition = 'ts>=1619 and ts<=1619452799999 limit 111'
        sql = "select * from {} where {}".format(self.tradeDB.table_name, condition)
        print(sql)
        result = pd.read_sql(sql=sql, con=self.tradeDB.conn)
