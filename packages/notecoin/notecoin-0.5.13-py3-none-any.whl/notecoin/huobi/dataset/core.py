...
class AutoSaveDetail(SqliteTable):
    ...
    def auto_save(self):
        time_unix = datetime.datetime.now() + timedelta(days=-1)
        if self.save_freq in ('d', 'D', 'Day', 'day'):
    ...
    def _save(self, pop=None, now=None):
        now = now or (datetime.datetime.now() + timedelta(days=-1))
        ...
        self.to_csv(condition=condition, path=path)
        if pop:
