...
* echo $(cat v2.6.5/ip) forumv2.6.5.example.com | sudo tee -a /etc/hosts
* firefox http://forumv2.6.5.example.com # user: api, password: BefShnygs33SwowCifViwag
* tox
...
* When the sidekiq queue grows too much http://forumv2.6.5.example.com/sidekiq shows which jobs are waiting
...
* tests/build-discourse figure out which commit hash of discourse_docker matches
  a given version and add it using v2.6.5 as a model
