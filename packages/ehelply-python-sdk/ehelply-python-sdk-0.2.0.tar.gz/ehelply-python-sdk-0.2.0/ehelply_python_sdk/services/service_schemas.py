...
async def transform_response_to_schema(response: ClientResponse, schema: Union[None, Type[HTTPResponse]]) -> GenericHTTPResponse:
    if response.status == 200:
        if schema:
            result: Union[list, dict] = await response.json()
    ...
    if response.status == 403:
        return ErrorHTTP403Response(**await response.json())

    if response.status == 404:
        return ErrorHTTP404Response(**await response.json())

    if response.status == 409:
        return ErrorHTTP409Response(**await response.json())

    if response.status == 422:
        return ErrorHTTP422Response(**await response.json())
...
def is_response_error(response: Union[ClientResponse, HTTPResponse]) -> bool:
