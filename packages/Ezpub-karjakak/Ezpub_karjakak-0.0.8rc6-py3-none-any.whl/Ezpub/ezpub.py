...
def build(path: str):
    ...
    if os.path.isdir(path):
        ...
        if fda not in os.listdir():
            os.mkdir(fda)
        folds = [f for i in ['dist', '.egg-info'] for f in os.listdir() if i in f]
        if folds:
            fda = os.path.join(fda, f'{str(dt.timestamp(dt.now())).replace(".", "_")}')
            os.mkdir(fda)
            try:
                for i in folds:
                    AttSet(i).set_file_attrib(AttSet.FILE_ATTRIBUTE_READONLY)
                    shutil.move(i, fda)
                AttSet(fda, True).set_file_attrib(AttSet.FILE_ATTRIBUTE_READONLY)
                pnam = f'py -m build'
                with Popen(pnam, stdout = PIPE, bufsize = 1, universal_newlines = True, text = True) as p:
                    for line in p.stdout:
                        print(line, end='')
