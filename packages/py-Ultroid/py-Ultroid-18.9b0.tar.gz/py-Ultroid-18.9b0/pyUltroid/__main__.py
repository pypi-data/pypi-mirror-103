...
for name in files:
    with open(name) as a:
        ...
        try:
            if BOT_MODE and plugin_name in BOTINVALID_PLUGINS:
                LOGS.info(
                    f"Ultroid - Official - BOT_MODE_INALID_PLUGIN - {plugin_name}"
                )
            else:
    ...
    for name in files:
        with open(name) as a:
            ...
            try:
                if BOT_MODE and plugin_name in BOTINVALID_PLUGINS:
                    LOGS.info(
                        f"Ultroid - Addons - BOT_MODE_INALID_PLUGIN - {plugin_name}"
                    )
                else:
...
if Plug_channel and not BOT_MODE:
    ...
    LOGS.info("Plugin Channel - Denied")
...
if BOT_MODE:
    LOGS.info("Ultroid - BOT_MODE - Started !")
