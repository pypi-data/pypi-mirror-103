...
def exec_(ctx, cmd_name, extra_args, command, save_as, warn, mount, interactive, tty, user, group):
    ...
    if user is None:
        try:
            proc = subprocess.run(['id', '-u'], stdout=subprocess.PIPE, check=True)
            user = proc.stdout.decode().strip()
        except subprocess.CalledProcessError:
            pass

    if group is None:
        try:
            proc = subprocess.run(['id', '-g'], stdout=subprocess.PIPE, check=True)
            group = proc.stdout.decode().strip()
        except subprocess.CalledProcessError:
            pass
