...
from ourcompose.framework import Composition, Piece, Dependency, Competition, piecify, depend
...
class TestFramework(IsolatedAsyncioTestCase):
    ...
    async def test_piecify_pieces_functions_will_pass(self):
        
        class Comp(Composition):

            @piecify()
            async def fn0(self, l: list) -> list:
                return l

            @piecify()
            @depend(on="fn0", connected_to="l")
            async def fn1(self, l: list) -> list:
                return [x+1 for x in l]

        cmp = Comp("my-comp")
        result = await cmp.perform({'l': [1,2,3]})
        self.assertEqual(result, [2,3,4])

    async def test_piecify_pieces_and_run_batches_will_pass(self):
        ...
        cmp = Comp("my-comp")
        result = await cmp.perform([
            {'l': [1,2,3]},
            {'l': [2,3,4]},
            {'l': [3,4,5]},
        ])
        self.assertEqual(result, [[2,3,4],[3,4,5],[4,5,6],])

    async def test_piecify_pieces_with_complex_dependencies_will_pass(self):
        ...
        class Comp(Composition):
            ...
            @piecify()
            async def fn2(self, l: list) -> list:
                return l

            @piecify()
            @depend(on="fn0", connected_to="l0")
            @depend(on="fn1", connected_to="l1")
            @depend(on="fn2", connected_to="l2")
            async def fn3(self, l0: list, l1: list, l2: list) -> list:
                return [
                    l0[i]+l1[i]+l2[i] 
        ...
        self.assertEqual(result, [[3,6,9],[6,9,12],[9,12,15],])