...
def is_google_cloud() -> bool:
    ...
    try:
        ...
        response = requests.get(
            ...
            timeout=timeout,
            headers={'Metadata-Flavor': 'Google'},  # noqa: WPS432
        )
...
def _is_docker_from_cgroup() -> bool:
    return _in_cgroup('/docker')


def _is_docker_dockerenv() -> bool:
    return Path('/.dockerenv').exists()


def is_docker() -> bool:
    return _is_docker_dockerenv() or _is_docker_from_cgroup()


# This is experimental, we only return True if we're sure
# but we could have false negatives
def is_docker_build() -> bool:
    return _in_cgroup('/docker/buildkit')
